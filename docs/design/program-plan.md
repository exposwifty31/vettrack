...
- **A → native foundation:** the experience model (Phase 2) is written as **UI-framework-free contracts** (pure TS — no React/DOM/wouter imports) so the native implementation can consume it as spec — concretely the Expo/React-Native app + `@vettrack/contracts` in the sibling repo `exposwifty31/literate-dollop` (per `docs/MAINTENANCE_MODE.md`), not an abstract future target. The costliest part of going native is re-deriving product logic tangled into web UI; this de-tangles it up front.
...
### Phase 3 (A2) — Per-role nav + home split v1 (ops vs floor) — ✅ HOME SPLIT SHIPPED 2026-07-07 (PR #53; 9-agent design workflow + 16-agent adversarial review [fence+hook-safety clean, 6 findings fixed]; nav deltas #1/#3 deferred — see Deferred owner questions)
...
- **R1 — FLOW_INVENTORY live-walk stamping (owner-gated).** `docs/audit/FLOW_INVENTORY.md` still stamps every live walk `⏳ pending`; green CI ≠ walked UX. Backstopped by Phase 10's full four-platform re-verification — this row exists so the debt is *scheduled*, not silently deferred. If run pre-Phase-10: reuse a running dev server (never `pnpm dev` fresh — its predev kills ports 3001/5000 and would kill a concurrent agent's server), record which uncommitted work each walk exercised.
- **R2 — Tasks patient-field residue (owner-sanctioned; sequence AFTER PR #61 merges).** The booking form's `animalId`/`ownerId` fields are half-repurposed as device labels; the audit flags residual patient-domain form scaffolding. Client-render cleanup only — `vt_appointments` schema, `/api/appointments`, and `appointmentsPage.*` keys stay frozen. Touches `src/pages/Tasks.tsx` → **coordinate with PR #61's owner to avoid a re-edit collision**; if that page is also the >800-LOC split candidate below, do the residue cleanup first, split second.
- **R3 — Dead `TodayScreen` entry (verify-first).** Audit lists a dead `TodayScreen` nav/route entry. **Honor "don't remove reachable pages"** ([[feedback_no_removing_core_pages]]): prove genuine deadness (zero reachable nav path + zero deep-link) before removal; if reachable, un-guard instead. Routes to the phase that owns the nav models (`nav-model.ts` + `native-nav-model.ts`) or folds into Phase 8; not a capability-only stub now.
- **R4 — Disposition intentional-no-op reconciliation (docs, not removal).** The audit's "disposition stub workers (inventory-deduction) with Removal Protocol" **conflicts with a standing owner decision**: the inventory-deduction no-op is *intentionally preserved* post-migration-143 (`docs/scope-change-2026.md`; pre-Phase-7 cleanup Group B kept it). Resolution here is a one-line reconciliation note in the audit + `FLOW_INVENTORY`, NOT a code removal — surfacing the conflict, not acting on the audit's suggestion.
- **R5 — Historical-doc noise trim (low, docs-only).** Trim/ban­ner the design-handoff + pre-2026 historical docs the audit flags as noise, matching the IMP-003 banner pattern from the cleanup. Append-only to `PROOF_ALIGNMENT_LOG`.
- **R6 — `Tasks.tsx` >800-LOC split (structural, after R2).** Behavior-preserving component extraction mirroring 7S (move-not-rewrite, byte-identical behavior, its own PR). Blocked on R2 so the residue is gone before the file boundaries move.
...
# Owner-added roadmap items (captured 2026-07-15)

> Two items the owner flagged as missing. Rationally adapted to the current codebase — **not** copied verbatim from the source references. Neither is slotted into the Phase 0–10 structure yet (placed here for owner triage/sequencing), and **neither blocks the App Store resubmission** — both are quality/feature follow-ons.

- **RM-1 — NFC tag payloads: standardize on HTTPS universal links, not `vettrack://`.** VetTrack reads NFC three ways and only one path needs changing. **In-app scanning (primary)** already runs a foreground CoreNFC session via the Capacitor NFC plugin → reads tags directly, **no iOS notification, no tap** ✅. **Background tag-in-hand (app closed)** is the gap: iOS auto-reads the NDEF payload and, for a **custom-scheme** URI (`vettrack://…`), shows a notification that **requires a tap** (and may not offer it at all for an unrecognized scheme); for an **HTTPS universal link** (`https://vettrack.uk/…`) it **opens the app directly, tap-free** via the already-live AASA (T-13). **Action:** encode NFC tag payloads — especially the **dock master tags** (docking P2 NFC path) and equipment/inventory tags — as HTTPS universal-link URLs mapping to the existing deep-link routes (`/equipment/:id`, dock-return, restock), and confirm `apple-app-site-association` covers those paths. Tap-free hospital scan = a real App-Store-review quality signal. Sequence near the native-migration / NFC surfaces; not a resubmission blocker.

- **RM-2 — Real-time collaboration (Socket.IO) = R-RTC-1, already specced.** Socket.IO for VetTrack is **only** the additive, ephemeral collaboration channel: live typing/presence on shift-chat, `/board` co-presence + cursors, and record co-presence. It **never** carries domain or emergency state — SSE + `vt_event_outbox` stays the sole transport, and Code Blue is never put on a lossy socket (**Globally-frozen: "no transport replacement"**). Design is **done**: `docs/plans/consolidated-audit-10x/subspecs/R-RTC-1-realtime-collaboration.plan.md` (merged #97; cards R-RTC-1.1–1.7 — DB-read handshake auth, CSWSH/origin defense, socket-lease presence store, `@socket.io/redis-adapter` scaling, graceful degradation, required emergency-isolation test). **Sequencing:** deferred until the docking work merges — it wires a Socket.IO namespace onto the same `server/index.ts` HTTP server the docking branch is actively editing, so serialize to avoid the collision. A **post-docking (likely post-resubmission) build**, not a resubmission blocker.
...
| Capacitor near limit; Expo/RN next | ✅ real, documented | `MAINTENANCE_MODE.md` → `literate-dollop` + `@vettrack/contracts` | Already I.2 + Phase 10 roadmap (now names the real target) |
