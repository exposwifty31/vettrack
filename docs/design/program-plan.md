...
### Phase 0 — Baseline: audits, flow inventory, role-testing enabler — ✅ DONE (role switcher + `FLOW_INVENTORY.md`; live-walk executed 2026-07-16)
...
### Phase 7R (Remediation) — Post-archaeology cleanup (before Phase 8) — ✅ DONE (#61/#71–#74; R1 flow-walk executed 2026-07-16)
...
- **R1 — FLOW_INVENTORY live-walk stamping — ✅ EXECUTED 2026-07-16.** Formerly the debt: `FLOW_INVENTORY.md` stamped every walk `⏳ pending` (green CI ≠ walked UX). **Now done** on all four surfaces via the executable harness (`tests/flow-walk/`, branch `claude/flow-walk-fixes`): web+board **147 rows / 145 pass / 0 broken / 2 degraded** across 5 roles, native **iPhone 68/68**, native **iPad 68/68**. Rows stamped, evidence in `docs/audit/evidence/`, account in `PROOF_ALIGNMENT_LOG.md`. Run via `pnpm dev:walk` (**never** plain `pnpm dev` — the per-IP rate limiter poisons auth mid-walk; `dev:walk` sets the sanctioned `PLAYWRIGHT_E2E=true` skip).
...
### Phase 8 (A3) — Full per-role differentiation — ✅ DONE (5 archetype home surfaces wired; caps fully differentiated) · ⚠️ exact student scope = deferred owner-Q
> **Status (git-verified 2026-07-16):** all five archetype home surfaces ship — `FloorHomeSurface.tsx` forks `VetHomeSurface`/`TechHomeSurface`/`StudentHomeSurface` (the `homeSurface` dimension stays the 2-way `ops`/`floor` split; the floor surface differentiates the three floor archetypes internally). Capabilities are fully 5-archetype: `student` is DERIVED as `tech − WITHHELD_FROM_STUDENT` (a one-way ratchet, byte-identical to the pre-P8 empty grant). The one open item is the **exact student scope** — a deferred owner question the code ships a defensible default for (student = restricted tech), not a blocker.
