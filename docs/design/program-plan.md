...
- **`--status-stale` color (design drift A3, opened 2026-07-07) — ✅ DECIDED 2026-07-08: PURPLE.** The Phase-1 mock uses purple `#AF52DE` for stale custody; code's `--status-stale` was orange, aliasing maintenance. Resolution (pre-Phase-7 cleanup): a distinct purple `--status-stale` triplet now ships in `src/index.css` (light `279 68% 60%` + dark `279 70% 70%`, with matching `-bg/-fg/-border`), fixing the flagship-audit **M3** two-ambers collision. Tailwind already maps `stale: hsl(var(--status-stale))`, so no config change was needed. Phase 6/7 bind to the token; the 7c/7e readiness palette is unblocked.
