...
The sticker is an **entry point into the custody flow, never custody logic or custody authority**.
Fixed division of labor:

- **What the tap provides:** certain identification of the equipment item (UUID on the sticker) +
  immediate opening of the toggle offer — exactly the existing `nfcAction=toggle` contract. Actor
  identity comes from the **session** (the signed-in user on the phone), never from the tag: every
  checkout/return is recorded against an authenticated person, with `clinicId`, role, and
  server-side rate limits. In this respect an NFC tap is equivalent to a QR scan, minus camera
  friction.
- **What the tap does NOT decide:**
  1. **Docked ≠ returned.** For dock-managed categories, the docking event remains the canonical
     return signal (P1–P3 docking model). A sticker tap may *offer* a return; it complements the
     dock event, never replaces it.
  2. **Tap ≠ physical possession.** One can tap a sticker without taking the equipment — custody
     ...
     `stale-returned-sweep`, Equipment Coordinator escalation ladder, waitlist reservations), all
     unchanged.
  3. **The tag is not authority** — ADR-006's spirit (set for RFID, equally binding here): a tag is
     evidence/identification. NTAG215 is clonable by design choice; it never decides state. The
     server-side state machine is the single source of truth, including when a tap targets
     equipment currently held by another user.
- **Offline:** a tap with no network still opens the app (the universal-link association is stored
  by the OS); the page renders from the Dexie cache and the custody action joins the sync queue
  under the existing rules (the Code Blue offline exception is untouched).
...
4. **QA sample per batch** (before mass encoding the rest): one iPhone background-scan + one Android
   tap must both reach the right equipment page.
...
| 8 | Tap → toggle on equipment checked out by ANOTHER user | UI and server honor the server-side custody state — no silent override; the flow surfaces the current holder per existing behavior |
| 9 | Dock-managed equipment: sticker tap while item is out | Tap may offer return, but the docking event remains the canonical return signal (docked ≠ returned invariant unchanged) |
