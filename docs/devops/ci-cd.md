...
There is **no Staging environment** on Railway: `production` is the only environment of the VetTrack project (the staging tier was removed on 2026-08-19, see above), and neither production service has a GitHub source — every deploy goes through `deploy.sh`.
