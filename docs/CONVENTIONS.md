...
- `export *` is for **barrel/index modules only** — a file whose entire job is to re-export a
  ...
  **One documented exception: `server/db.ts`.** It is not a barrel — it creates the Drizzle
  pool — and it also re-exports `server/schema/`, which a large number of call sites import
  from. Read strictly, the rule above says to remove that re-export; do not. It stays for
