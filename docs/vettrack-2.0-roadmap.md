...
**This roadmap is the single authoritative 2.0 plan.** Every other 2.0 document layers under it by
reference — none is a competing plan:

- `.claude/docs/ai/vettrack/10x/session-2.md` — frozen owner-decision record (2026-07-16). Reference
  only; never rewritten, never flattened into this doc.
- `docs/plans/master-plan-2026-07.md` — implementation plan for the ready-now slice + the Layers 0–6
  program map (incl. Android-ship and bare-RN-migration sequencing).
- `docs/design/program-plan.md` — experience direction (per-role UX, management console, board target).
- `docs/plans/2.0/task-*.md` — per-task execution plans (the breakdown-first deliverables).

### Artifact map — where each completed task's deliverables live

Branch-side rows were verified locally on 2026-07-22 (`git show --stat` per commit) on the owner's
machine. **`claude/task-1.1-autopilot-shadow` is a local-only branch — it has not been pushed to
origin**, so its paths do not resolve in a fresh clone until it is pushed. That also means every
Phase-0 execution artifact below currently exists on a single machine — a durability risk that closes
only when the branch is pushed. Each Phase-0 task also has a PROOF_ALIGNMENT_LOG entry in the branch's
copy of the log.

| Task | Artifact | Location | Caveat |
|---|---|---|---|
| 0.1 | `docs/design/case-spine-allowlist.md` | local branch (5b18a691e) | — |
| 0.2 | `docs/plans/2.0/case-spine-spike-findings.md` | local branch (6c5699653) | spike code on `worktree-agent-ad05bf556984d8f59` |
| 0.3 | `docs/plans/2.0/autopilot-spike-findings.md` | local branch (8f52e1257) | spike code on `worktree-agent-a64779cdd0617e6ff` |
| 0.4 | `docs/design/autopilot-policy-layer.md` | local branch (18c9def43) | — |
| 0.5 | `docs/plans/2.0/autopilot-backtest.md` + `scripts/analysis/autopilot-backtest.ts` | local branch (179e5aeb1) | **SYNTHETIC harness — never cite for real thresholds** |
| 0.7 | code + `docs/capacitor-native-app.md` hygiene law | local branch (9766e548f) | on-device physical confirmation still OPEN |
| 1.1 plan | `docs/plans/2.0/task-1.1-autopilot-shadow.md` | local branch (45d429a85, c745707ac) | ACTIVE task |
...
Task 1.1 §0 was owner-resolved on the branch (commit c745707ac): **per-org policy gate +
auto-publish-on-timeout fallback**. Task 1.4's billing queue reuses this same per-org policy-gate
pattern (see Task 1.4's locked decision 2).

### Merge-reconciliation rule for `claude/task-1.1-autopilot-shadow`

Precondition: the branch must first be pushed from the owner's machine — until then there is nothing
on origin to reconcile. When its PR opens, it will carry **stale 18-task copies** of this roadmap and
`scripts/vettrack-2.0-scope-gate.sh` that add/add-conflict with main. Resolution is fixed in advance:

1. Both conflicts resolve toward **main's** versions (19-task tracker, strict-parser gate). Main's
   tracker is strictly ahead — the branch holds no tracker state main lacks.
2. Branch-side PROOF_ALIGNMENT_LOG entries **append** to main's log (append-only convention), never
   replace or reorder existing entries.
3. Everything else on the branch (the Phase-0 artifacts and task-1.1 plan in the table above) merges
   as-is — main deliberately holds references, not copies, of that content.
...
> re-litigate the critical finding or locked decisions below; they're owner-confirmed. **NOT STARTED —
> spec only, no implementation yet.**

- **Objective:** Attribute consumable supplies (syringes, gauze, drugs, gloves) to a specific
  patient/encounter with minimal manual staff input, so usage → patient → billing is captured without
  burdening clinicians. **This is a reconnection project, not a greenfield build:** ~80% of the pipeline
  already exists in the codebase, disconnected when ER/patient tables were removed (migrations 142–143).
- **Pre-built-but-disconnected pieces (verified, do not rebuild):** `vt_dispense_events` /
  `dispense.service.ts` (works, decrements stock, records dispenser — **no patient/case column**);
  `DispenseSheet.tsx` (patient picker exists but `activePatients` is hardcoded `[]` at line 161;
  `animalId` is threaded to the server then **forced to `null`** at `containers.ts:499/535/816` and
  `dispense.service.ts:374/414/452` — nowhere to persist it); `vt_inventory_item_prices` (full
  context-aware price table, zero read call-sites); `autoBilledCents` (declared, never incremented —
  dead "₪X captured" toast, `DispenseSheet.tsx:182`); `exportBillingEntry`
  (`generic-pms.ts:177`) + `VetTrackBillingEntry` + `exportBilling` config flag + dashboard tile (full
  outbound-to-PMS billing contract, zero callers); `evaluateClinicalInvariant` /
  `evaluateDispenseAgainstOrders` (complete `off|shadow|enforce` envelope, but the checked function is
  stubbed to return `{ orphanLines: [] }` — `ORPHAN_DISPENSE_BLOCKED` can never fire today); scan infra
  (`ScanScreen`, `use-scan-accountability`, `vt_scan_logs` — equipment-only today, but
  `vt_items.nfcTagId` / `vt_containers.nfcTagId` columns exist unused). There is **no patient/animal/case
  table** (removed in 142–143) — this task's Phase 0 creates one, scoped per this program's owner
  decision #1 (operational-only, no PHI). ⚠️ **Reconcile with 1.2 before either executes its own Phase
  0:** 1.2 (The Case Spine) also creates `vt_cases` — this task must extend/reuse 1.2's table and its
  allowlist (0.1), never stand up a second, competing `vt_cases`. Whichever of 1.1/1.4/1.2 lands its
  schema first owns the migration; the others add nullable FKs onto it.
- **Locked decisions (owner-confirmed, binding — do not revisit):**
  1. **Attribution = ambient case set by one scan of a patient/kennel tag** — not active-task, not
     shift-worklist, not a manual picker as the primary path. Scanning a case tag opens an ambient case
     context; every subsequent dispense stamps that `caseId`. Manual picker is a rare fallback only.
  2. **Billing = capture → priced `PENDING_REVIEW` queue → approve → `exportBillingEntry` to PMS**, with
     a per-org auto-publish-on-timeout policy (reuse task 1.1's per-org policy-gate pattern) — **not**
     straight-to-billing.
  3. **Enforcement = shadow first, per-org enforce later.** Repoint `evaluateDispenseAgainstOrders` to a
     new `NO_CASE_ATTACHED` orphan reason; ship in shadow; flip to enforce per-org only after shadow data
     proves scan-to-case is reliable.
- **Build order (this task's own internal phases — do not confuse with the roadmap's Phase 0–4 above):**
  ...
  - *Sub-phase 1 · Ambient scan-to-case (critical path):* branch the scan resolver on tag type
    ...
    dispense; rename the null-forced `animalId` plumbing to `caseId` and actually persist it; populate
    `DispenseSheet`'s `activePatients` from a real source (fallback path).
  - *Sub-phase 2 · Pricing + review queue:* resolve `vt_inventory_item_prices` at dispense time →
    populate `autoBilledCents`; add `PENDING_REVIEW` queue + approve UI + the per-org
    auto-publish-timeout policy; wire the existing `exportBillingEntry`.
  - *Sub-phase 3 · Shadow enforcement + reconciliation:* repoint `evaluateDispenseAgainstOrders` →
    `NO_CASE_ATTACHED` (shadow mode); add an expected-vs-actual variance view (reuse
    `restock.service.ts`'s `consumedDerived`).
  - *Sub-phase 4 (optional/later):* `vt_procedure_bundles` auto-emitting draft dispenses on task
    completion, for zero-touch low-cost-item capture.
  - Sub-phases 0→1 are the critical path; 2 and 3 are independent after 1.
- **Execute:** Per this program's own methodology — write `docs/plans/2.0/task-1.4-consumable-capture.md`
  (writing-plans style, sub-phase by sub-phase) before implementation; RED (failing test) before any
  implementation file, sub-phase 0 first.
- **Constraints (do NOT):** no PHI in `vt_cases` (opaque external refs only — patient name/species/owner
  never stored); do not rebuild pricing, billing-export, dispense, or the enforcement envelope — they
  exist, reconnect them; do not touch frozen surfaces (SSE/outbox transport, BroadcastChannel envelope,
  PWA build-tag, emergency-endpoint cache denylist, `appointmentsPage.*` / `vt_appointments` /
  `/api/appointments`); do not weaken Strategy A (resolver-throw degrades to `off`); no offline queuing
  for any Code-Blue/emergency path; the mis-attribution banner + idle-TTL is **mandatory**, not optional
  — no silent ambient state; every new/changed query filters by `clinicId`, no exceptions; no hardcoded
  copy — new strings in both `locales/he.json` + `en.json` via the typed `t.*` accessor, Hebrew default.
- **Verify:** Per sub-phase: failing test written before its implementation; `npx tsc --noEmit` (both
  tsconfigs) to 0 errors before done; PROOF_ALIGNMENT_LOG entry per task with real evidence
  (Read/grep/run, not summary); frozen-surface diff review.
- **Done when:** Sub-phases 0–1 (critical path) land and reviewed; 2–3 may ship independently after.
  **Depends 0.1 (allowlist), 1.2 (Case Spine — schema reconciliation, see above). Reuses 1.1's per-org
  policy-gate pattern for billing auto-publish. Not started — spec only as of 2026-07-21.**
...
(6) **`vt_cases` ownership** — 1.2 (Case Spine) and 1.4 (consumable-usage capture) both plan to create
`vt_cases`; whichever lands its schema first owns the migration, the other extends it with nullable
FKs — resolve explicitly before either starts its own Phase 0, don't let two competing tables ship.
...
also not session-2. The thesis wording
