...
2. Find the current tail on BOTH refs and take the max — the checkout alone is not enough
   when the branch is behind: `ls migrations/*.sql | grep -v '\.down\.sql$' | sort -V | tail -1`
   and `git ls-tree -r --name-only origin/main migrations | grep -v '\.down\.sql$' | sort -V | tail -1`
   (`.down.sql` companions are excluded — the runner skips them, so they must not steer numbering).
   The next number is `max(both) + 1` (~~`185_rls_role_precondition_guard.sql` as of 2026-08-19~~ — **run the command; do not trust a number written here.** *Corrected 2026-08-27:* the tail is `186_vt_event_outbox_clinic_seq.sql` on this branch and **188** on `origin/main` `bf1e0480c`, which this branch is behind. Picking 186 from the stale line would collide.)
...
`migrations/meta/` — drizzle-kit's `_journal.json` plus three snapshots — was **deleted on 2026-08-19**. It had stopped at `0020` in April 2026 while the directory grew to ~~188 `*.sql` files (187 of them applied; `185_rls_role_precondition_guard.sql` is the highest-numbered)~~ **189 top-level `migrations/*.sql` files, 188 of them applied** — one down-migration is skipped — **with `186_vt_event_outbox_clinic_seq.sql` the highest-numbered (*corrected 2026-08-27*; on `origin/main` `bf1e0480c` it is 191 files, 190 applied, tail 188). Counts are top-level only, which is all the runner reads: `migrations/manual/` is a subdirectory and is ignored**, `server/migrate.ts` never read it, and stale bookkeeping that nobody maintains is worse than none. If drizzle-kit is ever re-adopted, regenerate a baseline against the real database rather than restoring those files.
