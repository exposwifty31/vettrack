...
|---|--------|------|---------------------------|--------|------------------|-------|-------------------|-------------------------------|--------------|
| 1 | `POST` | `/scan` | `requireAuth` → `checkoutLimiter` → `requireEffectiveRole("student")` → `validateBody(quickScanBodySchema)` | **No** | **No** — client `equipmentApi.quickToggle()` is **online-only** (no `addPendingSync`; NFC deep-link + foreground reader) | Yes (`equipment_checked_out` / `equipment_returned`, `via: quick_scan`) | No | Light (inline checkout/return + `scanLogs` + `undoTokens`; return branch inserts `equipmentReturns` with `isPluggedIn: true`) | **Paused — high** (handler extraction still blocked; **client consumer shipped** for NFC toggle) |
