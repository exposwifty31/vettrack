...
|---|--------|------|---------------------------|--------|------------------|-------|-------------------|-------------------------------|--------------|
| 1 | `POST` | `/scan` | `requireAuth` → `checkoutLimiter` → `requireEffectiveRole("student")` → `validateBody(quickScanBodySchema)` | ~~Yes~~ **No** — *Corrected 2026-08-28: the route is registered WITHOUT `equipmentReplayIdempotency` and the registry has no `quickScan` entry (verified against `server/routes/equipment.ts` and `server/lib/equipment-replay-idempotency.ts` on `origin/main`). A duplicate delivery can therefore double-toggle custody; closing this is the D2 server half, tracked as its own change.* | **No** — legacy pilot/demo path only; **NFC** uses `equipmentApi.quickToggle()` → `POST /:id/toggle` (online-only, full V1 custody) | Yes (`equipment_checked_out` / `equipment_returned`, `via: quick_scan`) | No | Light (inline checkout/return + `scanLogs` + `undoTokens`; return branch inserts `equipmentReturns` with `isPluggedIn: true`) | **Paused — high** (handler extraction still blocked; NFC does **not** call this route) |
...
- **Replay registry** (`server/lib/equipment-replay-idempotency.ts`): `create`, `update`, `delete`, `checkout`, `return`, `toggle`, `seen`, `scan` (`POST /api/equipment/:id/scan`), `quickScan` (`POST /api/equipment/scan`).
  *Corrected 2026-08-26 (this line previously read ~~"`POST /api/equipment/scan` (quick-scan) is **not** in the replay endpoint map"~~):* S16.2 added the `quickScan` key and mounted `equipmentReplayIdempotency` on the flat alias, closing the last unwrapped custody mutation on this router. `quickScan` is a **separate** key from `scan` on purpose: the replay row is looked up by `(clinic_id, key)` and guarded by a request hash over method + URL + body, so the two routes can never serve each other's cached body — but `endpoint` is the label written to `vt_idempotency_keys` and emitted as `route` on collision telemetry, and reusing `scan` would file every quick-scan replay under the per-id route.
- **Why the quick-scan alias matters more than its traffic suggests:** it is a *toggle* (available → checkout, held-by-caller → return). An unguarded duplicate delivery does not repeat the first effect, it **undoes** it — a replayed "check this out" silently becomes "return it". Every other wrapped route replays into the same state; this one replays into the opposite one.
- **Server-side only until a client sends the header.** `equipmentReplayIdempotency` calls `next()` when `Idempotency-Key` is absent, so today's callers see no behaviour change: no web/Capacitor client calls this route at all (`tests/nfc-quick-toggle-client.test.ts` pins that NFC uses `POST /:id/toggle` instead), and the RN client's `api.equipment.scan` is online-only and sends no key. The guard becomes live only with a companion RN change — send a stable `Idempotency-Key`, or route the hero scan through the offline queue, which mints and persists one key per enqueued mutation.
