...
| **Presence** — `PATCH /api/code-blue/sessions/:id/presence` | **Any authenticated user** whose clinic owns the session (`requireAuth` only — **no** clinical-authority gate). Both the vet and the technician heartbeat here. | `UPSERT vt_code_blue_presence` on `(sessionId, userId)` → bump `lastSeenAt` → `code_blue_presence_joined` audit **on the first beat of a (session, user) pair only** — `RETURNING (xmax = 0)` distinguishes the insert from the update, later beats write no audit row (#250, 2026-09-11; the per-beat `code_blue_presence_heartbeat` kind stays in the union for history). | **200** `{ok:true}`. | Session not in the caller's clinic / bad id → **404** `SESSION_NOT_FOUND`. No role/shift/check-in prerequisite — presence never 403s on authority. |
