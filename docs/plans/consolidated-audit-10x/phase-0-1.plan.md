...
- **T-23 (R-EQ-F2 · small-02 readiness badge)** — five dispatchable cards, each ≤2 files + 1 test:
  ...
  - **T-23d** — the `<ReadinessBadge>` component (shape+glyph+text, from the tier helper). RED: `tests/readiness-badge.test.tsx` (**glyph/container contrast ≥3:1 and rendered-text contrast ≥4.5:1, both light and dark themes**; screen-reader label present).
  - **T-23e (mechanical mount fan-out — ONE card, exempt from the ≤2-file *implementation* bound because each change is a 1–2 line import+render, not logic):** mount `<ReadinessBadge>` at each named call site — **list (`my-equipment`/equipment-list), detail (`equipment-detail`), home (role surfaces), board (`src/board/*`), locate (`LocateSearch`)** — one import+render per surface. RED: `tests/readiness-badge-surfaces.test.tsx` (badge renders on each named surface). Verify: `pnpm test -- tests/readiness-badge-surfaces.test.tsx && pnpm typecheck`.
- **T-24 (R-EQ-F3 · small-04 damaged-at-check-in)** — five dispatchable cards, each ≤2 files + 1 test (net-new-data feature: full checklist per spec §2.5):
  ...
  - **T-24e** — **GREEN:** readiness rules read the new `conditionStatus` so a damaged device reads not-ready. RED: **its own** `tests/damage-readiness-not-ready.test.ts` (distinct from T-24b's route test) — after a damage report the readiness rules report the device not-ready. Verify: `pnpm test -- tests/damage-readiness-not-ready.test.ts && pnpm typecheck`.
...
- **T-30 (R-IN-F1 · small-03 nudge)** — three cards (≤2 files each): **T-30a1 (backend · nudge feed):** route `expiryCheckWorker`/`restock.service` output to a role-scoped nudge feed. RED: `tests/expiry-nudge-feed.test.ts` (correct role only; other roles excluded). **T-30a2 (telemetry):** the bounded-enum nudge telemetry — closed enum on client + `server/routes/realtime.ts` + the `incrementMetric()` union. RED: `tests/expiry-nudge-telemetry.test.ts` (out-of-enum rejected on client + server). **T-30b (UI):** the dismissible home-surface nudge (dismiss persists). RED: `tests/expiry-nudge-ui.test.tsx`. **T-30c (push):** once-per-event push via `notification.worker`. RED: `tests/expiry-nudge-push.test.ts`. No new realtime path.
