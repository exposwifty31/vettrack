...
- **GREEN (grace retains FIRST, then confirm):** a null keepalive **within** the `RECONCILE_GRACE_MS` grace window is **ignored** — the session is retained, and a refetch is **not** allowed to clear during grace even if it would return null. **Only after the grace window elapses** may a **confirming refetch** run, clearing only if it too returns no active session. **Boundary (pinned):** keepalive age **`< RECONCILE_GRACE_MS` = within** (retain; no clearing refetch) and age **`>= RECONCILE_GRACE_MS` = after** (a confirming refetch may run). Order is strict: grace-retain precedes the confirming refetch; the refetch never short-circuits the grace.
- **RED:** `tests/code-blue-null-keepalive-grace.test.tsx` (fake timers) — a null keepalive at age **`RECONCILE_GRACE_MS − 1`** (within) **does not clear even when a refetch would return null**; at **exactly `RECONCILE_GRACE_MS`** (the `>=` boundary) a confirming refetch runs and clears only on a confirmed null; no clearing refetch is issued during grace.
