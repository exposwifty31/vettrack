...
- **a) schema (directional + tenant-safe at the DB boundary):** `vt_rfid_readers` (`clinicId, id, name, gatewayCode, roomId FK, fromRoomId FK, toRoomId FK, physicalLocation, status, lastSeenAt, provisioningState, createdAt`). **Tenant-safe constraints (pinned — enforced in the DB, not just service queries):** **composite unique `(clinicId, gatewayCode)`** (not a bare "unique per clinic"); `roomId`/`fromRoomId`/`toRoomId` must reference rooms in the **same `clinicId`** — enforce via a tenant-safe composite FK (`(clinicId, roomId) → vt_rooms(clinicId, id)`) or a validated CHECK; **reject partial or self-referential directional pairs** (a gate has both `fromRoomId`+`toRoomId` or neither; `fromRoomId ≠ toRoomId`). Migrate; `npx drizzle-kit generate` → commit SQL. **RED:** `tests/migrations/rfid-readers.test.ts` (DB-integration) — directional columns exist; **a duplicate `(clinicId, gatewayCode)` is rejected; a cross-clinic `roomId`/`fromRoomId`/`toRoomId` is rejected; a self-referential or half-populated directional pair is rejected**.
