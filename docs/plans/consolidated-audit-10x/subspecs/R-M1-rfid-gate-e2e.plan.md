...
- **a) schema (directional + tenant-safe at the DB boundary):** `vt_rfid_readers` (`clinicId, id, name, gatewayCode, roomId FK, fromRoomId FK, toRoomId FK, physicalLocation, status, lastSeenAt, provisioningState, createdAt`). **Tenant-safe constraints (pinned — enforced in the DB, not just service queries):** **composite unique `(clinicId, gatewayCode)`** (not a bare "unique per clinic"); `roomId`/`fromRoomId`/`toRoomId` must reference rooms in the **same `clinicId`** — enforce via **composite foreign keys** (`(clinicId, roomId)`, `(clinicId, fromRoomId)`, `(clinicId, toRoomId)` each → `vt_rooms(clinicId, id)`; add `UNIQUE (clinicId, id)` on `vt_rooms` if absent), **not a CHECK** (a CHECK can't enforce cross-row referential integrity); **reject partial or self-referential directional pairs** (a gate has both `fromRoomId`+`toRoomId` or neither; `fromRoomId ≠ toRoomId`). Migrate; `npx drizzle-kit generate` → commit SQL. **RED:** `tests/migrations/rfid-readers.test.ts` (DB-integration) — directional columns exist; **a duplicate `(clinicId, gatewayCode)` is rejected; a cross-clinic `roomId`/`fromRoomId`/`toRoomId` is rejected; a self-referential or half-populated directional pair is rejected**.
...
- **c) resolver uses direction** — "exited ER → entered Ward" produces last-seen = destination room. **Egress signal (pinned, idempotent):** an exit through a boundary/dock gate with **no matching entry** emits a bounded-enum **`possible_egress`** signal surfaced by R-M1.3 (NOT a separate alert, NOT deferred). **Correlation (pinned):** correlation key = `(clinicId, equipmentId, boundary-gate)`; matching uses a **bounded ordering/time window**; **retries and out-of-order RFID batches are deduplicated** against persisted correlation state, so exactly one signal fires per genuine egress. **RED:** `tests/rfid-direction-resolve.test.ts` — directional resolve to the destination room; a boundary-exit-without-entry emits **exactly one** `possible_egress`; **a retry emits no duplicate; a reordered/out-of-order batch produces no duplicate or contradictory signal**.
...
- Board shows RFID **last-seen + `rfid_reader_offline` + `rfid_location_conflict` + `ambiguous_rfid_location` + `possible_egress`**, each asserted on the snapshot, and **none of these signals overrides custody state** (the human-confirmed location stays authoritative); `/api/display/snapshot` still uncached.
