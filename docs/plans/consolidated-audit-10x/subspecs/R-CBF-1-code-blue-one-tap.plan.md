...
- **RED:** `tests/code-blue-one-tap-orchestration.test.ts` — one call yields a reserved nearest-ready cart + a session linked to it + an outbox event + an enqueued page; a **duplicate token replays the same session with NO second cart reservation attempted**; **two concurrent starts with the same token resolve through the single duplicate-replay path**; **an aborted transaction leaves no partial session/reservation**; cross-clinic isolation asserted.
