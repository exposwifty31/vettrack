...
- **Reuse anchors (shared; substantial scaffolding already exists — verify at authoring):** `server/services/asset-copilot-orchestrator.service.ts` + `asset-copilot-resolve.service.ts`; `server/routes/equipment-copilot.ts`; `server/domain/equipment/copilot/{answer.types,ai-safety-validator,citation-validator}.ts`; `server/domain/equipment/evidence/resolver/*`; `docs/archive/2026/PH-01-operational-assistance-during-chaos.md`.
