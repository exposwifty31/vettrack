...
- **Spec:** `../../superpowers/specs/2026-07-12-audit-10x-consolidated-plan-design.md` · **Preceded by:** Phase 0+1 (`phase-0-1.plan.md`) — do those first (stabilize before extend).
...
- **Tier (model routing):** **default = S (Sonnet).** `⚠ FROZEN` (inline or inherited from a `⚠ FROZEN` section header) sets a **floor of at least `Tier: S +R`**; an inline tag may **raise** it (to `O +R`) but never **lower** it below the floor — so a frozen card can never resolve to plain `S` (README → "Deterministic routing rule"). `+R` = a `code-reviewer` gate (+ browser drill for realtime/PWA) before commit. See README → "Execution driver".
...
- **T-35 (R-SC-03 · CLICK-PATH-016):** `src/components/nfc-foreground-scan.tsx:48` (`handleEquipmentId` at `:46`) — the sessionStorage re-fire guard is stamped (`markNfcToggleFired`, `:49`) before `runEquipmentQuickToggle` (`:52`) and never cleared on that call's failure paths (`src/lib/nfc-equipment-toggle.ts` `runEquipmentQuickToggle` `:61` — network `:86-88`, 409 `:90-95`, throw `:97`), so the 8 s success guard (`NFC_TOGGLE_GUARD_TTL_MS = 8_000`, `src/lib/nfc-equipment-toggle.ts:9`; checked by `wasNfcToggleFiredRecently` `:23`) silently drops a retry for 8 s after any failure. **GREEN:** on every failure path clear the 8 s success guard via a new `clearNfcToggleFired(equipmentId)` in `src/lib/nfc-equipment-toggle.ts`, but still suppress hardware reader re-fires (multiple `onRead` per physical tap) with a **pinned short debounce — a named constant `NFC_REFIRE_DEBOUNCE_MS = 500` (500 ms), distinct from the 8 s `NFC_TOGGLE_GUARD_TTL_MS`**: after a failure a re-fire within `NFC_REFIRE_DEBOUNCE_MS` is still dropped, a genuine user retry at or after it proceeds. Two impl files (`src/lib/nfc-equipment-toggle.ts` + `src/components/nfc-foreground-scan.tsx`). **RED:** `tests/nfc-toggle-failure-guard.test.tsx` (fake timers) — a network/409/throw failure clears the 8 s lockout, and asserts the debounce **boundary**: a retry at `NFC_REFIRE_DEBOUNCE_MS - 1` (499 ms) is suppressed while a retry at `NFC_REFIRE_DEBOUNCE_MS` (500 ms) fires exactly once. **Verify:** `pnpm test -- tests/nfc-toggle-failure-guard.test.tsx && pnpm typecheck`.

### Sync banner + SW update — `⚠ FROZEN` (PWA)

> The section `⚠ FROZEN` flag is a **floor**, inherited by every card below (README §"Deterministic routing rule" 1): each resolves to **at least `Tier: S +R`** — an inline tag may raise it but never lower it, so neither card can run as plain `S`. Both take the doctrine check; the SW-reload card (T-37) additionally takes the live PWA browser drill.

- **T-36 (R-SY-02 · CLICK-PATH-013) `⚠ FROZEN`:** `src/components/sync-status-banner.tsx:64` — `dismissed` is component-local with no reset; after one dismissal later permanent sync failures ("billing leakage") stay hidden all session. **GREEN:** key the dismissal to the **failure signature** — **defined as `(syncErrorKind, targetResource)`** (the error kind + the resource/mutation it concerns; two failures share a signature **iff both fields are equal**) — dismissal state is per-signature, not a component-local boolean; a distinct signature re-shows the banner. **RED:** `tests/sync-banner-redismiss.test.tsx` — dismissing a signature hides it and an **identical** signature stays hidden (persistence); a signature differing in **either** field re-shows the banner. **Tier: S +R** (frozen PWA floor made explicit — Sonnet + `code-reviewer` + doctrine check).
...
- **T-38 (R-SY-04 · CLICK-PATH-026):** `src/app/routes.tsx:161` — `/equipment/scan|maintenance|intelligence` alias redirects are shadowed by the dynamic `/equipment/:id` (L153/154). **GREEN:** move the three alias redirects **above** the dynamic route. **RED:** `tests/equipment-alias-redirects.test.tsx` (each alias resolves to its target, not the `:id` page). This is a pure wouter route-order fix — **no** SSE/SW/cache/Code-Blue surface — so the `⚠ FROZEN` floor does **not** apply; floor = `S` (default, no inline tag).
...
- `⚠ FROZEN` cards additionally pass their doctrine check; the cards with a **live SW / Code-Blue surface** (here: T-37's SW-update reload) also pass the browser drill. A frozen card with no live transport/SW surface (T-36's banner-dismissal state) takes the doctrine check + `code-reviewer` only.
