...
- Harness discovery live: after writing SKILL.md the skill appeared in the available-skills list, and `Skill(vettrack-team)` invoked successfully this session, rendering the full roster + benched list.
- Dry-run routing: "Clerk sign-in broken on iPhone shell" → Systematic Debugger + Clerk/Mobile Masters; "clean dead files" → The Janitor; "make the board prettier" → UI Master + Clinical Safety veto — all leads present in roster, all reference files exist.
