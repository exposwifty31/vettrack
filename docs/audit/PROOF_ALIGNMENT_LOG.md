...
**SKIPPED — owner action required (not silently patched):** the `RESUBMISSION_RUNBOOK.md §E` App-Review credential (`reviewer@vettrack.uk` / `VetTrack2026!`) is committed in plaintext AND hardcoded in `verify-resubmission.sh:48` AND in git history. Removing it from the runbook alone is a misleading half-fix (still live + exposed elsewhere), and rotating a credential is an owner action, not mine. **Surfaced to owner:** rotate the demo account, then remove from both files together (or move to a `REVIEWER_PASSWORD` env the script reads). Not changed in this batch.
