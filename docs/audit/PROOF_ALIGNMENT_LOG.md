...
**Not done (scoped out, flagged honestly):** the `DrizzleStartClaimStore` SQL is covered by construction + source-level assertions (clinic-scoping + fence-guard + `onConflictDoNothing`), not a live-DB round-trip — it exercises against Postgres only once R-CBF-1.1's endpoint composes it inside the session transaction. No Playwright drill (no browser surface in this sub-card; that is R-CBF-1.5's bar once the endpoint + client land).
