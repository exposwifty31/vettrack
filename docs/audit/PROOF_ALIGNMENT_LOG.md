...
## 2026-07-03 — Codebase relevance audit: verified dead-code removal (branch `chore/relevance-audit-cleanup`)

**Claim:** Removed only verified-dead files (root artifacts, dead frontend/backend modules, committed test artifacts) and the tests that covered them; every deletion was confirmed by importer analysis, not knip/intuition alone.

**Evidence (this session):**
- **False positives caught before deletion (the reason nothing live was removed):** a first-pass `rg` for inbound imports used the wrong path form and reported `src/native` (975 LOC), `src/core`, `src/infrastructure`, `src/shared`-family as "0 importers." Re-checking with the real `@/`→`src/` alias flipped all of them to LIVE (native←8, core←7, shared←14, shell←4, desktop←3, infrastructure/platform←1). knip also false-flagged `server/seed.ts` (live via `seed:dev` script) and `shared/permissions.ts`'s neighbor. Only barrels + genuinely orphaned files were removed.
- **Deleted (all verified 0 real importers):** Batch A root artifacts (`Archive.zip`, `Archive 2.zip`, `all-files.md`, `screenshot.png`, `app-tour.js`, a session .txt dump, `.nvrmc`); Batch B frontend (`src/features/today/*`, `design-system-entry.ts`+`AlertCard`+`ShiftProgressHero`, `lib/camera.ts`, `lib/constants/regex.ts`, `lib/task-dashboard-filters.ts`, `types/billing.ts`, `hooks/use-is-mobile.ts`, `native/NativeScreen.tsx`, `src/shared/`, root `shared/permissions.ts`); Batch C `src/infrastructure/{db,api,auth,index.ts}` (kept live `platform/`, confirmed it has no intra-dir deps on the removed siblings); Batch D `server/integrations/{rollout,conflicts}/*` (patient scope, removed in migs 142–143) + `inventory-deduction` worker+queue stub (0 callers, `@deprecated` no-op).
- **Untracked (Batch E):** `git rm --cached playwright-ui-screenshots/*` (38 png + index.html) — regenerated by `tests/ui-smoke.spec.ts`, already in `.gitignore`/`.railwayignore`; files kept on disk.
- **Test-web fallout handled:** the full `pnpm test` run surfaced 7 test files importing the deleted modules via *relative* paths (which the src-scoped/alias greps missed) — deleted 2 whole-dead test files, surgically kept the live assertions in 5 others (`core-ports-contracts`, `legacy-worker-warning`, `f1-server-metrics`, `f2b-job-registry-readiness-metrics`, `vt-appointments-container-id-schema`). `obsolete-feature-flags.test.ts` needed no change (its scan roots are `fs.existsSync`-guarded).
- **Gates (actually run this session):** `pnpm typecheck` (frontend+server) → **exit 0**; `pnpm typecheck:server` per-batch → exit 0; `pnpm test` → **384 files / 3788 tests passed, exit 0**; `pnpm build` → **exit 0** (built in 7.2s; chunk-size warning pre-existing). 6 commits on `chore/relevance-audit-cleanup`.

**Verdict:** VERIFIED at gate + full-suite + build level. NOT done this session (deliberately gated on explicit user go): the Batch A git-history purge of the two 95MB zips — `git-filter-repo` is not installed and it force-pushes to two remotes (gitlab + github), so it is held pending confirmation. Two follow-ups noted: dedup `.agents`/`.codex`/`.claude` skill mirrors, and investigate files newly-orphaned by the `design-system-entry.ts` removal (knip informational, nothing broken).

## 2026-07-03 — Correction: `design-system-entry.ts` + `AlertCard` + `ShiftProgressHero` were NOT dead (restored)

**Claim:** Three files deleted in Batch B were a false positive and have been restored on `chore/relevance-audit-cleanup`; the rest of the cleanup stands.

**What went wrong:** `src/design-system-entry.ts` has zero *app* imports — but it is the **design-sync export entry**, declared as `"entry": "src/design-system-entry.ts"` in `.design-sync/config.json`, and it re-exports `AlertCard` / `ShiftProgressHero` (both in the config's `componentSrcMap`). Its own header says "generated by /design-sync and is NOT used by the app itself." My relevance check verified `vite.config`/`package.json`/`index.html`/`src` as reference roots but **missed `.design-sync/config.json`** — so the external-tool consumer was invisible. Deleting these would have broken the DS bundle sync to claude.ai/design (110 components). The user caught it on the `chore/design-sync-conventions-refresh` branch before merge.

**Evidence (this session):**
- `.design-sync/config.json` → `"entry": "src/design-system-entry.ts"`; `componentSrcMap` maps `AlertCard` → `src/components/alerts/AlertCard.tsx`, `ShiftProgressHero` → `src/components/home/ShiftProgressHero.tsx`.
- Blast radius bounded to exactly these 3: none of the other Batch B/C/D deletions appear in `componentSrcMap` or the DS entry's `export *` list (checked the full 111-entry map + the entry file).
- Restored from `origin/main` (all 3 confirmed byte-identical to `origin/main`). Commit `6cae5478`.
- **Gates re-run after restore:** `npx tsc --noEmit` → **0 errors**; `pnpm build` → **success** (7.06s); `pnpm test` → **384 files / 3788 tests passed**.

**Verdict:** Corrected + re-verified. Lesson for future relevance audits: **treat `.design-sync/config.json` (`entry` + `componentSrcMap`) as a reference root** — a file with zero app imports can still be load-bearing for the design-sync tooling. Ideally register `src/design-system-entry.ts` as a knip entry so knip stops false-flagging the ~60 DS-only components (follow-up).
