...
## 2026-07-18 — R-SH-F1.1 · Schema (vt_shift_handover) shaped for Priza (TDD)

**Claim:** Built the shift-handover artifact schema foundation per docs/plans/consolidated-audit-10x/subspecs/R-SH-F1-shift-handover.plan.md §R-SH-F1.1. New `vt_shift_handover` table (migration 177) with the persisted key `(clinicId, shiftSessionId, revision)` (monotonic revision int; current = max revision), an explicit clinic-scoped `notification_read_at` read-state column (NULL = unread), and a PMS-agnostic discriminated `patientWorklist` contract with a zod persistence serializer. Added 3 members to the closed `AuditActionType` union (generate/acknowledge/unconfirm). No generator/route/surface (cards R-SH-F1.2–.5).

**Evidence:**
- **RED first (right reason):** `tests/shift-handover-contract.test.ts` (vitest) → `(0 test)` / `Cannot find module '../server/lib/shift-handover.js'`. `tests/migrations/shift-handover.test.ts` (tsx, DB reachable at localhost:5432) → `AssertionError: expected vt_shift_handover table to exist` (actual null). Both failed for the intended reason (module + table absent) before any impl.
- **GREEN (minimal):** created `server/lib/shift-handover.ts` (zod discriminated union + `serializePatientWorklist` + `CrossClinicWorklistError` + delta/open-item/observed-signal types, imports only zod — no DB, no cycle); added `shiftHandover` table + `ShiftHandoverRow`/`NewShiftHandoverRow` to `server/schema/ops.ts` (re-exported via `server/db.ts` → `schema/index.js`), consuming the contract types via `import type`; hand-authored `migrations/177_vt_shift_handover.sql` (additive, `CREATE TABLE/INDEX IF NOT EXISTS`, clinic-scoped unique key + current-revision index, `shift_session_id` FK-free mirroring `vt_shift_messages`, no FK to RFID/removed patient tables); added `shift_handover_generated|acknowledged|unconfirmed` to the closed union in `server/lib/audit.ts`.
- **Migration applied:** `pnpm db:migrate` → `✅ Applied migration: 177_vt_shift_handover.sql` (local DB was at 176 — RFID 172–176 present locally; 177 unique regardless of merge order).
- **Guardrails asserted by RED:** discriminated union is keyed on `state` (type test: ready has no `code`, error has no `entries`, not_configured is a bare discriminator — `@ts-expect-error` on excess props, gated by tsc); `error` is distinguishable at runtime from `not_configured` AND from a `ready` empty list (an error can never collapse to empty); the serializer REJECTS an unknown `error.code` (throws) and STRIPS an unsafe adapter `message`/`url`/`credential` before persistence (`toEqual({state:'error',code:'unreachable'})`, serialized output contains no `SECRET-TOKEN`/`priza.example`/`message`); a mixed-clinic `ready` entry whose `byTechId` is not in the target clinic's user set throws `CrossClinicWorklistError` (never persisted); the migration asserts the unique constraint on `(clinic_id, shift_session_id, revision)` (duplicate triple rejected, revision 2 coexists) and that `notification_read_at` exists, is nullable, and defaults NULL.
- **Frozen guardrails intact:** every column carries an explicit target-table `clinic_id NOT NULL` predicate; unique key + lookup index both clinic-scoped; `clinicId` is system-derived (no request input in this card — schema only); no new realtime path/transport; the Priza contract is external-ids + display only (no internal patient model, no FK to removed tables); ack is a deliberate confirm reversible via the persisted unconfirm (schema supports it: acknowledgedBy/At + notificationReadAt all clearable); no hardcoded Hebrew (no `.ts`/`.tsx` copy — locales untouched).
- **Commands:** `pnpm exec tsx tests/migrations/shift-handover.test.ts` → `✅ shift-handover.test.ts passed`. `pnpm exec vitest run tests/shift-handover-contract.test.ts` → `Tests 7 passed (7)`. `pnpm typecheck` (frontend `tsc --noEmit` + server `tsconfig.server.json`) → exit 0. `pnpm test` → `Test Files 616 passed (616)`, `Tests 5539 passed | 11 skipped`, 0 failed. `pnpm depcruise:check` → `no dependency violations found (915 modules)` (schema→lib type-only import introduced no cycle/boundary violation).

**Verdict:** VERIFIED

## 2026-07-18 — R-SH-F1.2 · Delta generator (all 4 types) at shift end (TDD)

**Claim:** Built the in-process `generateShiftHandover(clinicId, shiftSessionId, opts?: { regenerate?: boolean })` generator per docs/plans/consolidated-audit-10x/subspecs/R-SH-F1-shift-handover.plan.md §R-SH-F1.2. It aggregates the 4 locked delta types (custody / task-state / alerts / dispenses) from the EXISTING `vt_audit_logs` + `vt_event_outbox` over the shift window `[start, end)` into `deltas` + `openItems`, reusing the `ShiftHandoverDeltas`/`ShiftHandoverDeltaEntry`/`ShiftHandoverOpenItem` types + `serializePatientWorklist` from SH-1.1 (no forked contract). Idempotent per `shiftSessionId`: the no-opts (scheduler) path returns the persisted current (max-revision) row unchanged if one exists (same `generatedAt`, no duplicate deltas) else inserts `revision = 1`; `{ regenerate: true }` inserts `max(revision)+1` preserving priors. Wired the no-opts call into `server/app/start-schedulers.ts` via a new `startShiftHandoverScheduler` (scans recently-ended `vt_shift_sessions`, system-derived `clinicId`, NO public generate route). `patientWorklist` left `{ state:'not_configured' }` (R-SH-F1.4 populates it via the PMS port).

**Evidence:**
- **RED first (right reason):** `tests/shift-handover-generator.test.ts` (vitest, `describe.skipIf(!DATABASE_URL)`, DB reachable at localhost:5432) → `(0 test)` / `Cannot find module '../server/lib/shift-handover-generator.js'` before the impl existed. Then after DB seeding a `no_delete_audit_logs` append-only-rule collision surfaced in the test's own purge (expected env gotcha), fixed by DISABLE/ENABLE around the audit purge; the 6 cases then went GREEN.
- **GREEN (minimal):** `server/lib/shift-handover-generator.ts` — explicit audit-action Sets per dimension + a normalized-keyword fallback for heterogeneous outbox `type`s (`classifyDeltaKind`), skips the `audit_log` outbox wrapper, resolves the window from `vt_shift_sessions` (legacy id) or a roster `win:` id, derives open items as the non-terminal latest-by-target task/alert deltas; idempotent insert with a unique-collision re-read guard. `server/lib/shift-handover-scheduler.ts` + registration in `server/app/start-schedulers.ts`.
- **Guardrails asserted by RED:** seeded shift lists EXACTLY the seeded in-window deltas across all 4 types (custody = 1 audit + 1 outbox, task = started+completed, alert = 1, dispense = 1); deltas before/after `[start,end)` excluded; open items = non-terminal task1 + unresolved alert1 only (completed task2 + custody/dispense excluded); CROSS-CLINIC negative — clinic-B same-looking audit+outbox rows never leak into clinic-A's artifact and vice-versa (target-table `clinicId` predicate on every read); retry returns the SAME row id + revision 1 + identical `generatedAt` (getTime equal) + identical deltas + still exactly ONE persisted row; `{ regenerate:true }` inserts revision 2 with the prior revision 1 still readable, and a subsequent no-opts retry then returns the new current revision 2.
- **Frozen guardrails intact:** every audit/outbox/session/handover read carries an explicit `clinicId` predicate; deltas read ONLY from the existing `vt_audit_logs` + `vt_event_outbox` — no new realtime path/transport; generation `clinicId` is system-derived (scheduler reads it off the ended-session row) — NO public `/generate` route; reused SH-1.1's contract types + `serializePatientWorklist` unchanged; no reintroduced internal patient model (worklist stays `not_configured` here); no hardcoded Hebrew (open-item summaries are internal `kind:targetId` strings, no `.ts` copy).
- **Commands:** `pnpm typecheck` (frontend `tsc --noEmit` + server `tsconfig.server.json`) → exit 0. `pnpm test -- tests/shift-handover-generator.test.ts` → `Tests 6 passed (6)`. `pnpm test` (full) → `Test Files 617 passed (617)`, `Tests 5545 passed | 11 skipped`, 0 failed. i18n:check N/A (no locale/copy touched). Orphaned `test-shf-%` clinics from the RED purge-collision run were swept from the dev DB post-GREEN.

**Verdict:** VERIFIED

## 2026-07-18 — R-SH-F1.3 · App-observed signals (TDD)

**Claim:** Extended `generateShiftHandover` per docs/plans/consolidated-audit-10x/subspecs/R-SH-F1-shift-handover.plan.md §R-SH-F1.3 to populate `observedSignals` with SYSTEM-DERIVED observations (app-observed custody/scan/readiness events) attributable to the shift window `[start, end)`, beyond the manually-logged audit/outbox deltas. Read on its OWN clinic-scoped read path over the EXISTING `vt_scan_logs` table (separate from the delta aggregation over `vt_audit_logs` + `vt_event_outbox`), reusing the `ShiftHandoverObservedSignal` type from SH-1.1 (no forked contract). Each signal = `{ sourceId: scan id, kind: "scan:<status>" (bounded, from the scan's own status column — no PII/free-form), at: ISO timestamp }`.

**Evidence:**
- **RED first (right reason):** `tests/shift-handover-observed.test.ts` (vitest, `describe.skipIf(!DATABASE_URL)`, DB reachable at localhost:5432) → `Tests 2 failed | 1 passed (3)`: `expected [] to have a length of 2` and the in-window ids assertion `+ []` — `observedSignals` was hardcoded `[]` in the generator before this card. The "excludes outside window" case passed trivially against the empty array (correct — nothing to exclude yet). Failed for the intended reason (collector absent) before any impl.
- **GREEN (minimal):** `server/lib/shift-handover-generator.ts` — added `collectObservedSignals(clinicId, window)` reading `vt_scan_logs` with an explicit `clinicId` predicate + `[start, end)` window bounds (`gte(timestamp, start)`, `lt(timestamp, end)`), ordered oldest→newest, mapped to `ShiftHandoverObservedSignal`; wired it into the generate path (`observedSignals` now collected and inserted instead of `[]`). Imported `scanLogs` from `../db.js` and `ShiftHandoverObservedSignal` type from `./shift-handover.js`. Updated the module docstring (observed-signals path is now populated, not empty).
- **Guardrails asserted by RED:** seeded in-window scans (2) surface as observed signals with the exact ids + bounded `scan:ok`/`scan:blocked` kinds + the seeded ISO timestamp; scans BEFORE (`07:00`) and AFTER (`17:00`, == exclusive end past) `[start,end)` excluded; CROSS-CLINIC negative — clinic-B's same-looking in-window scans never leak into clinic-A's artifact (`observedSignals` length exactly 2 for A; none of B's ids present in A) — the target-table `clinicId` predicate holds on the observed read path too.
- **Frozen guardrails intact:** the observed read carries an explicit `vt_scan_logs.clinicId` predicate; reads ONLY the existing scan-logs table — no new realtime path/transport; generation `clinicId` stays system-derived (this card adds no route); reused SH-1.1's `ShiftHandoverObservedSignal` type unchanged (no forked contract); no reintroduced internal patient model (scan events reference `equipmentId`, not patients); `kind` is a bounded `scan:<status>` label — no PII, no raw free-form text; no hardcoded Hebrew (no `.ts`/`.tsx` copy — locales untouched).
- **Commands:** `pnpm typecheck` (frontend `tsc --noEmit` + server `tsconfig.server.json`) → exit 0. `pnpm test -- tests/shift-handover-observed.test.ts` → `Tests 3 passed (3)`. `pnpm test -- tests/shift-handover-generator.test.ts` (regression on the shared generator) → `Tests 6 passed (6)`. `pnpm test` (full) → `Test Files 618 passed (618)`, `Tests 5548 passed | 11 skipped`, 0 failed. i18n:check N/A (no locale/copy touched).

**Verdict:** VERIFIED

## 2026-07-18 — R-SH-F1.4 · Patient/animal worklist via the PMS integration port (adapter-agnostic) (TDD)

**Claim:** Populated `patientWorklist` through a PMS-AGNOSTIC port per docs/plans/consolidated-audit-10x/subspecs/R-SH-F1-shift-handover.plan.md §R-SH-F1.4. Added a `PatientWorklistProvider` capability (`getPatientWorklist(credentials, window)`) to the existing `IntegrationAdapter` contract (`server/integrations/adapters/base.ts`) and a new `server/integrations/patient-worklist-port.ts` that resolves the clinic's adapter PER-CLINIC from `vt_integration_configs` (`enabled` + `syncPatients`, explicit `clinicId` predicate) via the existing `getAdapter` registry + `getCredentials`, calls the PORT (never a named PMS), and maps the outcome onto the SH-1.1 discriminated `PatientWorklist` union: no enabled syncPatients adapter → `{ state:'not_configured' }`; adapter throws → `{ state:'error', code }` (closed enum, raw detail dropped); entries → `{ state:'ready', entries }`. `generateShiftHandover` now calls `resolvePatientWorklist` and validates via the existing `serializePatientWorklist` against the clinic's `vt_users` id set before persist. Priza stays one reference adapter; the seam is adapter-agnostic (any adapter implementing the port plugs in).

**Evidence:**
- **RED first (right reason):** `tests/shift-handover-patient-worklist.test.ts` (vitest, `describe.skipIf(!DATABASE_URL)`, DB reachable at localhost:5432) → `(0 test)` / `Cannot find module '../server/integrations/patient-worklist-port.js'` before the port existed. Failed for the intended reason (port module absent) before any impl.
- **GREEN (minimal):** created `server/integrations/patient-worklist-port.ts` (`PatientWorklistWindow`, `PatientWorklistProviderEntry`, `PatientWorklistProvider`, `PatientWorklistCapableAdapter`, `PatientWorklistProviderError` carrying a closed `code`, and `resolvePatientWorklist(clinicId, window, deps?)` with injectable `resolveAdapter`/`loadCredentials` seams defaulting to `getAdapter`/`getCredentials`); added the optional `getPatientWorklist?` method (type-only import — no runtime cycle) to `server/integrations/adapters/base.ts`; wired `server/lib/shift-handover-generator.ts` to load the clinic tech-id set (`loadClinicTechIds`, explicit `clinicId` predicate on `vt_users`), call `resolvePatientWorklist` (threading `opts.worklistDeps`), and `serializePatientWorklist` the result before persist — replacing the hardcoded `{ state:'not_configured' }`.
- **Guardrails asserted by RED (4 cases):** a MOCKED adapter through the port populates the worklist per tech (`ready`, entries keyed by internal `byTechId` == seeded in-clinic `vt_users.id`); NO configured PMS → `patientWorklist` EXACTLY `{ state:'not_configured' }` (not an empty ready list) while `observedSignals` + `deltas.custody` still generate; a configured-but-FAILING adapter (throws `PatientWorklistProviderError('timeout')`) → `{ state:'error', code:'timeout' }` (not not_configured), the serialized artifact contains NO raw PMS detail ("raw PMS detail" string absent), and `observedSignals` + `deltas.custody` still generate; a SECOND, different adapter resolved through the SAME `resolvePatientWorklist` path yields the same `ready` shape — proving adapter-agnostic, not Priza-coupled.
- **Frozen guardrails intact:** the `vt_integration_configs` resolution + the `vt_users` tech-id read both carry an explicit target-table `clinicId` predicate; `patientWorklist` is the ONLY patient-data source (through the port — no reintroduced internal patient model, external ids + display only); the closed error-code enum + `serializePatientWorklist` strip guarantee no raw PMS message/url/credential reaches the artifact; a PMS failure carries ONLY into `patientWorklist` — deltas/open-items/observed still generate; reused SH-1.1's `PatientWorklist` union + `serializePatientWorklist` unchanged (no forked contract); no new realtime path/transport; generation `clinicId` stays system-derived; no hardcoded Hebrew (no `.ts`/`.tsx` copy — locales untouched). The RED custody signal is seeded via `vt_event_outbox` (cascade FK) rather than the append-only `vt_audit_logs` to keep the fixture purgeable.
- **Commands:** `pnpm typecheck` (frontend `tsc --noEmit` + server `tsconfig.server.json`) → exit 0. `pnpm test -- tests/shift-handover-patient-worklist.test.ts` → `Tests 4 passed (4)`. `pnpm test` (full) → `Test Files 619 passed (619)`, `Tests 5552 passed | 11 skipped`, 0 failed. i18n:check N/A (no locale/copy touched).

**Verdict:** VERIFIED

## 2026-07-18 — R-SH-F1.5 · Surface — extend /handoff + acknowledge + push (TDD)

**Claim:** Built the `/handoff` handover surface + the acknowledge/unconfirm mutations + the once-only next-shift push per docs/plans/consolidated-audit-10x/subspecs/R-SH-F1-shift-handover.plan.md §R-SH-F1.5. ONE shared next-shift-roster helper (`server/lib/shift-handover-roster.ts` — pure `pickNextShiftUserIds` + DB `resolveNextShiftRoster`, explicit `clinicId` predicate on both reads, roster↔user by the existing `normalizeName`/`normalizeNameKey` match) drives BOTH push-target selection on generate AND acknowledge authorization, so push-targets ≡ ack-authorized set by construction. `generateShiftHandover` fires the push ONCE on a genuine NEW-revision insert (injectable `notifyDeps`; default builds a `push_to_user` job with an idempotencyKey + `push.handover.*` copy at the recipient locale) — never on the retry-returns-existing path. `server/services/shift-handover.service.ts` adds `acknowledgeHandover` (records `acknowledgedBy`+`acknowledgedAt`, flips `notificationReadAt`→read, NO push), `unconfirmHandover` (clears the ack, restores `notificationReadAt`→null, writes its own `shift_handover_unconfirmed` audit row), `resolveAckAuthorizedUserIds` (== the roster helper off the handover's own shift-window end), and the `/handoff` read serializer. `server/routes/shift-handover.ts` (GET /current + POST/DELETE /:id/acknowledge, `clinicId` from auth only, per-locale `apiError()`) registered in `server/app/routes.ts`. Client: `src/components/handover-artifact-panel.tsx` (single `<h1>`, iPhone single-pane vs iPad two-pane, deliberate two-step confirm with `aria-pressed` + focus INTO confirm / RETURN to trigger, `<bdi>`-isolated LTR staff names, loading/error/empty/ready states each announced), `src/pages/handoff.tsx` (data + mutations + deep-link fallback to /home), typed `api.shiftHandover.{current,acknowledge,unconfirm}` + `src/types/shift-handover.ts`, `handoverPage.*` + `push.handover.*` copy in he/en parity.

**Evidence:**
- **RED first (right reason):** `tests/shift-handover-surface.test.tsx` → `Failed to resolve import "../server/lib/shift-handover-roster.js"` before any impl. After the modules landed, a real boundary bug surfaced (RED, right reason): the pure next-shift test failed `Set{} !== Set{noa,omri}` because a contiguous next shift starts EXACTLY at the outgoing shift's end (16:00) and the strict `>` comparison excluded it → fixed to `>=` (current shift always starts strictly before `afterEnd`, so it is never picked). Two further RED iterations reflected the append-only `vt_audit_logs` teardown gotcha (RESTRICT clinic FK) → mocked `logAudit` and asserted the unconfirm audit at the call boundary (`actionType:shift_handover_unconfirmed`, clinicId, targetId==handover id, performedBy==actor, `readStateTransition:read_to_unread`) per repo guidance.
...
## 2026-07-17 — R-BDF-1.2 · Board "attention" section (calm/pressure-aware, single-shot)

**Claim:** Rendered the R-BDF-1.1 anomaly set as RANKED, glance-only cards on the Command Board and added the pinned SINGLE-SHOT state machine. Ordering (pinned): severity (pressure>calm) → rule priority (`battery_critical` > `rfid_reader_offline` > `cart_unverified`) → `since` age (oldest first) → `unitId` (stable tie-break). calm mode stays quiet; pressure escalates color+size (and, on the one-shot activation only, motion). `prefers-reduced-motion` swaps motion for a static variant in BOTH modes (pressure still escalates color/size, no animation). State machine: dedup key = `(type,unitId)`; `absent → active (fire once) → cleared`; re-fire only on a subsequent `cleared → active`. Wired additively into `CommandBoard` (present in both modes) over the already-fetched `snapshot.anomalies` — no new transport, glance-only (zero interactive targets).

**Evidence:**
- **RED first (right reason):** wrote `tests/board-anomaly-statemachine.test.ts` (7 cases) + `tests/board-attention-render.test.tsx` (9 cases) BEFORE the modules existed. First run → `2 failed (2)`, `Failed to resolve import "@/features/command-board/components/BoardAttentionSection"` and the state-machine hook import — feature absent, right reason (no vacuous green).
- **GREEN (minimal):** added `board-anomaly-ranking.ts` (pure `rankBoardAnomalies`), `use-board-anomaly-state-machine.ts` (`useBoardAnomalyStateMachine` + `boardAnomalyKey`), `use-prefers-reduced-motion.ts`, `components/BoardAttentionSection.tsx`; re-run `pnpm test -- tests/board-anomaly-statemachine.test.ts tests/board-attention-render.test.tsx` → `Test Files 2 passed (2)`, `Tests 16 passed (16)`.
- **Ranking (pinned, order locked):** shuffled input `[cart_unverified, rfid_reader_offline, battery_critical(eq-9), battery_critical(eq-1)]` renders as `[battery_critical:eq-1, battery_critical:eq-9, rfid_reader_offline:rd-2, cart_unverified:cart-1]` (severity→priority→since→unitId). Equal-priority fixture (three `battery_critical`, differing `since`/`unitId`) locks to oldest-since-first then unitId (`eq-a`(08:00) < `eq-b`(09:00) < `eq-c`(09:00 tie→unitId)). Empty list → section renders nothing (`board-attention` absent).
- **calm vs pressure differ:** a `battery_critical` card is `data-anomaly-emphasis="quiet"` in calm mode and `"escalated"` in pressure mode; section `data-board-mode` reflects the mode.
- **reduced-motion (both modes):** motion-allowed → calm card `data-anomaly-motion="cross-fade"`, pressure card `"escalate"`. reduced-motion → `"static"` in BOTH; pressure+reduced STILL `data-anomaly-emphasis="escalated"` (color/size) with `data-anomaly-motion="static"` (no animation). Section `data-reduced-motion="true"`.
- **State machine (single-shot):** initial mount fires `onActivate` once per key (`justActivatedKeys` set, `activeKeys` set); an identical repeated snapshot (fresh array, same `(type,unitId)`) does NOT re-fire and `justActivatedKeys` is empty (held); a snapshot dropping the key clears it with no fire; reappearance re-fires ONLY after a clear (`cleared→active`); distinct `(type,unitId)` keys independent; a mode-driven re-render with the same keys does NOT re-fire; a severity flip on the same `(type,unitId)` does NOT re-fire (key is type+unitId only).
- **Glance-only guardrail:** `board-attention` section has zero `role="button"` and zero `button, a, input, [role='button'], [tabindex]` nodes — inert display only.
- **Wiring (additive):** `CommandBoard` computes `usePrefersReducedMotion()` + `board.anomalies ?? []` and renders `<BoardAttentionSection>` after the header (present in both calm and pressure bodies) only when anomalies exist; no existing panel altered.
- **Frozen guardrails held:** no new transport (derives from the already-fetched snapshot); `/api/display/snapshot` untouched → stays cache-denylisted; no new table; telemetry seam is the optional `onAnomalyActivated` prop (R-BDF-1.3 wires the bounded-enum counter — not added here); reduced-motion honored in calm AND pressure; single-shot escalation (no looping flash).
- **Commands:** `pnpm typecheck` (frontend `tsc --noEmit` + server `tsconfig.server.json --noEmit`) → exit 0, zero errors. `pnpm i18n:check` → EN/HE deep key parity (added `board.anomalyBatteryCritical`/`anomalyCartUnverified`/`anomalySince`, regenerated `i18n.generated.d.ts`). Board regression set (`board-attention-render`, `board-anomaly-statemachine`, `board-anomaly-rules`, `board-rfid-render`, `use-board-mode`, `command-board-panels`) → `Test Files 6 passed`, `Tests 56 passed`. Broad regression `pnpm test` → `Test Files 631 passed (631)`, `Tests 5667 passed | 11 skipped`, 0 failed.
- **Board Playwright drill (mandatory — board rendering changed):** built the frontend in dev-bypass (`env -u VITE_CLERK_PUBLISHABLE_KEY NODE_ENV=production pnpm build`, exit 0), started the API with `CLERK_ENABLED=false PORT=3001 NODE_ENV=development PLAYWRIGHT_E2E=true` (log: `[auth-mode] server mode=dev-bypass reason=clerk-explicitly-disabled`), waited for `/api/healthz`, then `PW_SUITE=board TEST_BASE_URL=http://127.0.0.1:3001 pnpm exec playwright test --project=chromium` → `2 passed (2.9s)` (BoardShell kiosk renders chrome-free + live snapshot poll). Confirms the additive attention section did not regress the live board render. Server stopped after the run.

**Verdict:** VERIFIED

## 2026-07-17 — R-BDF-1.3 bounded-enum board anomaly telemetry (uncommitted at log time)

**Claim:** Board anomaly types are a CLOSED enum on client + `server/routes/realtime.ts` + the `incrementMetric()` union in `server/lib/metrics.ts`. Each in-enum type maps 1:1 to a fixed metric id (`battery_critical`→`board_anomaly_battery_critical`, `rfid_reader_offline`→`board_anomaly_reader_offline`, `cart_unverified`→`board_anomaly_cart_unverified`), emitted single-shot per `(type,unitId)` activation via the R-BDF-1.2 `onActivate` seam. Out-of-enum types are rejected unconditionally on both client (never posted) and server (bumps `telemetry_payload_rejected_enum_mismatch`, no board counter).

**Evidence:**
- RED first: wrote `tests/board-anomaly-telemetry.test.ts`, ran before impl → `Tests 9 failed | 1 passed` with `TypeError: classifyBoardAnomalyType is not a function` / server `incrementMetric` "Number of calls: 0" (right reasons: missing client fns + missing server dispatch). The 1 pass = the absent→no-op case.
- `src/lib/realtime.ts:113-149` — `classifyBoardAnomalyType(type)` closed list `["battery_critical","cart_unverified","rfid_reader_offline"]` → returns type or null; `reportBoardAnomalyActivated(anomaly)` posts `{ boardAnomalyActivated: type }` only for in-enum, best-effort try/catch (never throws).
- `src/lib/api.ts:1241-1243` — `boardAnomalyActivated?: "battery_critical" | "cart_unverified" | "rfid_reader_offline"` added to the `realtime.telemetry` body type.
- `server/routes/realtime.ts` — `ALLOWED_BOARD_ANOMALY_TYPES` closed enum + `isAllowedBoardAnomalyType` guard; dispatch block maps each in-enum type 1:1 to its metric id, else `telemetry_payload_rejected_enum_mismatch`; `boardAnomalyActivated?: unknown` added to the parsed body shape.
- `server/lib/metrics.ts` — three ids added to the `MetricName` union AND `DEFAULT_COUNTERS` (`board_anomaly_battery_critical` / `_reader_offline` / `_cart_unverified`), matching the collab/rfid/sweep bounded-counter precedent (union + defaults, not the structured snapshot).
- `src/features/command-board/components/CommandBoard.tsx:485-490` — `<BoardAttentionSection onAnomalyActivated={reportBoardAnomalyActivated}>` wired additively; the state machine (`use-board-anomaly-state-machine.ts:58-66`) fires `onActivate` exactly once per absent→active, so emission is single-shot not per-snapshot.
- Test: `pnpm test -- tests/board-anomaly-telemetry.test.ts` → `Test Files 1 passed`, `Tests 10 passed` (client classifier, poster shape per type, unconditional out-of-enum client rejection, single-shot via state machine [once on activate / nothing on repeat / once on clear-then-reappear], server 1:1 metric id per type, server out-of-enum → enum-mismatch, absent no-op).
- Command: `pnpm typecheck` → exit 0 (frontend `tsc --noEmit` + server `tsconfig.server.json --noEmit`), zero errors.
- Command: `pnpm i18n:check` → `✓ locales/en.json and locales/he.json are in deep key parity` (no copy added — anomaly labels already exist from R-BDF-1.2; `git status locales/` clean).
- Regression: `pnpm test -- tests/board-anomaly-statemachine.test.ts tests/board-attention-render.test.tsx tests/board-anomaly-rules.test.ts tests/expiry-nudge-telemetry-server.test.ts` → `44 passed`; `tests/sw-api-cache-isolation.test.ts` + `tests/offline-phase-8-realtime-telemetry.test.ts` + `tests/sync-tel-realtime-telemetry.test.ts` → `11 passed` (emergency cache denylist + telemetry endpoint untouched).
- Broad regression: `pnpm test` → `Test Files 632 passed (632)`, `Tests 5677 passed | 11 skipped`, 0 failed.
- **Board Playwright drill (mandatory — CommandBoard wiring touched):** built frontend dev-bypass (`env -u VITE_CLERK_PUBLISHABLE_KEY NODE_ENV=production pnpm build` → exit 0), started API `CLERK_ENABLED=false PORT=3001 NODE_ENV=development PLAYWRIGHT_E2E=true tsx server/index.ts` (log: `[auth-mode] server mode=dev-bypass reason=clerk-explicitly-disabled`; `/api/healthz` → 200), then `TEST_BASE_URL=http://127.0.0.1:3001 PW_SUITE=board playwright test --project=chromium` → `2 passed (1.1s)` (BoardShell kiosk chrome-free + live snapshot poll). Server stopped after the run (`lsof -ti tcp:3001 | xargs kill`; health → 000).
- **Frozen guardrails held:** no new transport (telemetry derives from the already-fetched snapshot via the existing state machine); `/api/display/snapshot` untouched → stays cache-denylisted; bounded-enum only (closed union on client + server + `incrementMetric`); no new table; every anomaly source is the R-BDF-1.1 producer (clinic-scoped) — this card adds telemetry only; single-shot (once per activation); glance-only (no interactive targets added).

**Verdict:** VERIFIED

## 2026-07-17 — R-BDF-1.4 · Verification / acceptance bar (snapshot stays UNCACHED)

**Claim:** Card R-BDF-1.4 is the acceptance gate over R-BDF-1.1..1.3. It adds the one missing named acceptance test — `tests/sw-emergency-denylist.test.ts` — that proves BEHAVIORALLY (not just by source grep) that `/api/display/snapshot` is excluded from Cache Storage **reads AND writes** and that the emergency denylist path is untouched. The board-anomaly feature must never introduce caching of the snapshot to satisfy itself. No production code changed — this is a regression lock on the frozen SW emergency-bypass surface plus the acceptance-command roll-up.

**Evidence:**
- **RED first (genuine, right reason):** wrote `tests/sw-emergency-denylist.test.ts` as a live harness that evaluates the REAL `public/sw.js` in a `vm` sandbox with a mocked Cache Storage + fetch + `self`, capturing every `caches.open`/`cache.match`/`cache.put`/`cache.delete`. To prove the test is discriminating (not vacuously green), ran a throwaway experiment (`/tmp/red-check.mjs`, since deleted) against a MUTATED COPY of sw.js with `/api/display/snapshot` dropped from `EMERGENCY_BYPASS_PATHS` — the activate-purge assertion flipped to failure: `snapshot still cached after activate: true`, `delete counter: 0`, i.e. the test's `expect(cacheStore.has(SNAPSHOT_URL)).toBe(false)` would FAIL for exactly the right reason. The real (unmutated) frozen file was never touched.
- **GREEN:** `pnpm test -- tests/sw-emergency-denylist.test.ts` → `Test Files 1 passed (1)`, `Tests 6 passed (6)`:
  - snapshot fetch goes straight to network — `fetch` called once with the raw request, `caches.open`/`match`/`put` counters all `0`, nothing written to the mock cache (no read, no write);
  - bypass holds even with Vite-style `?v=`/`?t=` params (unconditional, runs before the dev-internals filter);
  - positive control — a normal static `/app.js` DOES open the cache (`open > 0`), proving the harness caches when the denylist does not apply;
  - activate purges a pre-seeded `/api/display/snapshot` entry while a benign `/app.js` entry survives (`delete > 0`);
  - source-level: `EMERGENCY_BYPASS_PATHS` present with every canonical `EMERGENCY_CACHE_BYPASS_PATHS` path verbatim; `/api/display/snapshot` is one of them.
- **Telemetry acceptance (mandatory, from R-BDF-1.3, re-verified as the gate):** `pnpm test -- tests/board-anomaly-telemetry.test.ts` → `Tests 10 passed` — asserts the exact 1:1 metric id per type (`battery_critical`→`board_anomaly_battery_critical`, `rfid_reader_offline`→`board_anomaly_reader_offline`, `cart_unverified`→`board_anomaly_cart_unverified`), single-shot emission (repeat snapshot emits nothing; clear-then-reappear re-emits once) through client `reportBoardAnomalyActivated` + `server/routes/realtime.ts` POST `/telemetry` handler + `incrementMetric()`, and unconditional out-of-enum rejection on both client (never posted) and server (bumps `telemetry_payload_rejected_enum_mismatch`, no board counter).
- **Seeded → right cards / healthy → none (from R-BDF-1.1/1.2, re-verified as the gate):** `pnpm test -- tests/board-anomaly-rules.test.ts tests/board-anomaly-statemachine.test.ts tests/board-attention-render.test.tsx` → `Tests 40 passed` — seeded anomalous state trips exactly the right ranked anomaly cards (severity→priority→since→unitId), a healthy clinic renders none, single-shot state machine holds, calm vs pressure differ, reduced-motion swaps motion for a static variant in BOTH modes.
- **Calm vs pressure spot-check:** covered in unit form by `tests/board-attention-render.test.tsx` — a `battery_critical` card is `data-anomaly-emphasis="quiet"` in calm and `"escalated"` in pressure; pressure+reduced-motion still `escalated` (color/size) with `data-anomaly-motion="static"` (no animation).
- **Board Playwright drill (mandatory — this card is on the board-rendering surface; NO dev server reachable at build time):** `:5000` and `:3001` both returned `000` (not running). Recorded exact drill command to run at the pre-PR gate against a dev-bypass server on :3001: build frontend dev-bypass (`env -u VITE_CLERK_PUBLISHABLE_KEY NODE_ENV=production pnpm build`), start API `CLERK_ENABLED=false PORT=3001 NODE_ENV=development PLAYWRIGHT_E2E=true tsx server/index.ts`, wait for `/api/healthz`, then **`TEST_BASE_URL=http://127.0.0.1:3001 PW_SUITE=board pnpm exec playwright test --config=playwright.config.ts --project=chromium`** (`PW_SUITE=board` → `tests/board-kiosk.spec.ts`, per `playwright.shared.ts`). Its most-recent green execution is logged under R-BDF-1.3 (`2 passed`); R-BDF-1.4 adds no board render change (test-only), so the render surface is byte-identical to the drilled R-BDF-1.3 state.
- **Commands:** `pnpm typecheck` (frontend `tsc --noEmit` + server `tsconfig.server.json --noEmit`) → exit 0, zero errors. Broad regression `pnpm test` → `Test Files 633 passed (633)`, `Tests 5683 passed | 11 skipped`, 0 failed. No copy added → `pnpm i18n:check` not triggered (`git status locales/` clean).
- **Frozen guardrails held:** `/api/display/snapshot` stays cache-denylisted (this card PROVES it, adds no caching); no new transport; bounded-enum telemetry only; motion clarifies (single-shot, reduced-motion honored both modes); every anomaly source clinic-scoped; no new tables; fail-safe derivation; glance-only; shares the R-M1.3 producer seam (`rfid_reader_offline` is one anomaly source, not a second producer).

**Verdict:** VERIFIED

## 2026-07-17 — R-RFID INT-1 · Land RFID controller package + header fix + schema parity

**Claim:** Integrate the forked RFID controller (built on PRE-R-M1 base `worktree-agent-acf91b7b4e276090b`) onto the current `claude/rfid-gates-board-alerts` branch (R-M1 + R-BDF-1). (a) bring `packages/rfid-controller/` (Modules 0-10) + `tests/rfid-rate-limiter-key.test.ts`; (b) keep the fork's redundant `server/routes/rfid-batch.schema.ts` absent (R-M1 already exports `RfidBatchSchema` from `server/routes/rfid.ts`); (c) header fix on R-M1 (one-`t` `x-vetrack-*` → canonical two-`t` `x-vettrack-*`) + extract a per-clinic limiter key; (d) update the header-fix test suites; (e) repoint the controller's parity test at the REAL exported schema and extend the controller contract with the R-M1.2a directional fields. Ingest behavior otherwise byte-identical (advisory-only, clinic-scoped, no new transport).

**Evidence:**
- **Package brought clean:** `git checkout worktree-agent-acf91b7b4e276090b -- packages/rfid-controller` (30 files: src Modules 0-10 + 14 test files + package.json/tsconfig/vitest.config) and `git checkout … -- tests/rfid-rate-limiter-key.test.ts`. Redundant `server/routes/rfid-batch.schema.ts` was never checked out → `ls` confirms `No such file or directory`.
- **(c) header fix — verified against real R-M1 files first:** pre-fix `server/routes/rfid.ts:49` read `req.headers["x-vetrack-clinic"]`, `:74` read `req.headers["x-vetrack-signature"]`, and `server/middleware/rate-limiters.ts:90` read `x-vetrack-clinic` (all one-`t`). Fix: both route reads → `x-vettrack-*`; extracted `export function rfidEventLimiterKey(req)` reading the two-`t` `x-vettrack-clinic` (trim; array-tolerant; per-IP tail only when the clinic header is truly absent) and set as the `rfidEventLimiter` keyGenerator. No other ingest behavior touched (grace-window secret loop, flag gate, schema parse, 422 directional reject all byte-identical). Left `server/integrations/webhooks/inbound.router.ts` (separate PMS-webhook surface, one-`t`) untouched — out of this card's scope.
- **RED→GREEN, limiter key (added behavior):** brought `tests/rfid-rate-limiter-key.test.ts` first → `pnpm test -- rfid-rate-limiter-key` = `TypeError: rfidEventLimiterKey is not a function`, `4 failed`. Implemented the extraction → suite green (verified in the combined run below). Asserts: keys `clinic-a:…` per-clinic for a two-`t` request; two clinics behind one IP get distinct keys; a one-`t`-only request does NOT key per-clinic; trims + tolerates array headers.
- **RED→GREEN, canonical-header authenticates (added behavior):** rewrote `tests/rfid-webhook-signature.test.ts` to canonical two-`t` throughout + added "canonical brand-cased `X-VetTrack-*` (two-`t`) headers authenticate → 202" and "the buggy one-`t` spelling is NOT accepted → 400 MISSING_CLINIC". Transiently reverting the route to one-`t` failed the new canonical cases (`5 failed | 2 passed`, all canonical/two-`t` requests → 400); restoring the two-`t` fix made them green.
- **(e) RED→GREEN, schema parity (added behavior):** repointed `packages/rfid-controller/tests/contract-parity.test.ts` from the deleted local schema to `import { RfidBatchSchema } from "../../../server/routes/rfid"` (stubbed `server/db.js` via `vi.mock`, matching the webhook test, so the real route schema imports without a DB — no `DATABASE_URL` mutation, so the DB-integration `e2e.test.ts` stays skipped) and added 11 directional battery cases (direction enum valid/invalid, gateway pair both/partial/out-of-range, all-three). Against the un-updated controller validator the 7 directional-reject cases failed (`7 failed | 37 passed`, cases #35,36,37,39,40,41,42 — controller stripped the unknown fields where R-M1 zod rejects). Extended `src/contract.ts` (`RfidBatchEvent` optional `direction`/`fromGateway`/`toGateway`, `RFID_DIRECTIONS` enum, `RFID_LIMITS.fromGateway/toGateway`, validator: enum check + 1..64 range + BOTH-or-NEITHER; `!== undefined` treats explicit null as present-and-invalid to match zod `.optional()`) → parity green.
- **Header-fix e2e callers realigned:** the pre-existing DB-integration e2e files `tests/rfid-gate-e2e.test.ts:133-134` and `tests/rfid-scan-only-golden.test.ts:228-229` POST with one-`t` headers; a transient baseline (route reverted to one-`t`) confirmed they passed on the old spelling (`9 passed`), i.e. the header fix — not a pre-existing break — required flipping their sends to two-`t`. Updated both (request sends only; the intentional one-`t` negative-assertion lines in the limiter/signature tests were left as-is).
- **Root scripts added:** `test:rfid-controller` = `vitest run --config packages/rfid-controller/vitest.config.ts`; `rfid-controller:typecheck` = `tsc --noEmit -p packages/rfid-controller/tsconfig.json`. (`pnpm-workspace.yaml` already globs `packages/*`; root tsconfigs include only `src`/`lib`/`server`/`shared*`, so the package does not enter root typecheck.)
- **Commands (final):**
  - `pnpm typecheck` (frontend `tsc --noEmit` + server `tsconfig.server.json --noEmit`) → exit 0, zero errors.
  - `pnpm test -- rfid` → `Test Files 24 passed (24)`, `Tests 135 passed (135)` (header-fix + limiter + gate-e2e + scan-only + all other rfid suites).
  - `pnpm test:rfid-controller` → `Test Files 14 passed | 1 skipped (15)`, `Tests 123 passed | 5 skipped (128)` (the skipped file = `e2e.test.ts`, DB-integration-class, self-skips with no `DATABASE_URL`).
  - `pnpm rfid-controller:typecheck` → exit 0.
- **Frozen guardrails held:** header fix changed only the header spelling + extracted a per-clinic limiter key — no other ingest behavior; every ingest read stays clinic-scoped; advisory-only (movement evidence, never custody/authority); no new transport; the controller ships zero runtime deps (global fetch); the redundant duplicate schema is gone so `RfidBatchSchema` has a single source (the route) with the parity test as its drift oracle.

**Verdict:** VERIFIED

## 2026-07-17 — R-RFID INT-1 (re-attempt) · Address reviewer findings on the landed controller

**Claim:** Resolve the three reviewer findings against the integration commit (`6267d5651`): (MEDIUM-1) the extended `contract.ts` directional fields were validated-for-parity but never emitted, leaving the advertised contract diverging from the wire payload as an *accidental* half-integration; (MEDIUM-2) three module headers (`direction.ts`, `envelope.ts`, `secret-source.ts`) made FALSE claims contradicting the very integration performed (claiming R-M1 had not landed, pointing at the DELETED `rfid-batch.schema.ts`, and claiming a silent-strip / single-secret behavior the real code refutes); (LOW) `debounce.ts` embedded a literal NUL (0x00) key separator, making git classify the file as binary. No ingest-surface behavior changes; advisory-only preserved.

**Evidence:**
- **MEDIUM-1 — resolved as a DELIBERATE, documented deferral (not wiring):** the controller cannot honestly emit the full directional trio — `direction` (entered|exited) requires gateway-role geometry that is hardware-track (ADR-004/006), explicitly out of this core — so emitting only a partial gateway pair would be incomplete evidence. Decision recorded in `envelope.ts` invariant #2 header: post-R-M1 the route schema AND the Module 0 contract DO accept the directional fields (validation parity, pinned by `contract-parity.test.ts`), but the controller deliberately emits the minimal safe `{tagEpc, gatewayCode, readAt}` and retains `MovementEvent.fromGateway` for logging only; `toWireEvent` can additively surface the pair when the hardware direction track lands, without touching validation. The contract fields are therefore NOT dead — they are the validation contract (drift oracle), and the deferral is now an explicit decision rather than an accidental gap. Locked by test: `envelope.test.ts` "emits ONLY {tagEpc, gatewayCode, readAt} on the wire — directional emission deferred" now asserts absence of `fromGateway`, `toGateway`, AND `direction` (strengthened from two to three) with the corrected rationale in-comment.
- **MEDIUM-2 — three false headers rewritten to post-R-M1 reality:**
  - `direction.ts`: removed "R-M1 has NOT landed … verified against `server/routes/rfid-batch.schema.ts`" (a file that does not exist on this branch); now states R-M1 HAS landed, `RfidBatchSchema` (server/routes/rfid.ts) accepts the optional directional fields, the contract validates the same shape, movement is internal evidence, and directional emission is deferred (cross-refs the envelope decision).
  - `envelope.ts`: removed the false "non-`.strict()` route schema would silently strip it, so emitting would make e2e falsely pass"; now states the schema explicitly ACCEPTS those fields (a partial gateway pair is a HARD reject, not a silent strip) and reframes invariant #2 as the deliberate deferral above.
  - `secret-source.ts`: removed "R-M1's M1.1c … has NOT landed — `verifyVetTrackWebhookSignature` checks exactly ONE secret"; now states rotation grace is SERVER-SIDE (`getRfidVerificationSecrets` returns `[current, previous]`, the route tries each), so the controller signs with one current secret and hot-swaps — the "controller never dual-signs" conclusion is unchanged, only the stale premise corrected.
  - Verification: `grep -rn "rfid-batch.schema" packages/ server/ tests/` → none; `grep -rn "has NOT landed\|silently strip" packages/rfid-controller/src` → none. (The remaining `contract.ts:178` "silently stripped" refers to genuinely-UNKNOWN fields, which non-`.strict()` zod does strip — accurate, not a flagged claim.)
  - These are comment-only rewrites on the frozen ingest surface — zero behavior change; the full package suite (below) stays byte-identical green.
- **LOW — NUL separator replaced with `\u001f` (ASCII escape):** `debounce.ts` key separator changed from a literal `\x00` byte to `const KEY_SEP = "\u001f"` written as the six-char ASCII escape so the SOURCE file is pure-ASCII and git treats it as text. Before: `file src/debounce.ts` → `data` (binary), `git diff` showed "Binary files differ". After: `file` → `…text`, `grep -Pc '[\x00]'` → `0` (no NUL byte remains). U+001F (Unit Separator) still cannot appear inside an EPC/gateway code, so the dedup key stays collision-free; `debounce.test.ts` → 4 passed (behavior unchanged). Documented rationale in the file header.
- **Commands (final):**
  - `pnpm typecheck` (frontend + server) → exit 0, zero errors.
  - `pnpm rfid-controller:typecheck` (`tsc --noEmit -p packages/rfid-controller/tsconfig.json`) → exit 0.
  - `pnpm test -- rfid` → `Test Files 24 passed (24)`, `Tests 135 passed (135)`.
  - `pnpm test:rfid-controller` → `Test Files 14 passed | 1 skipped (15)`, `Tests 123 passed | 5 skipped (128)` (skip = DB-integration `e2e.test.ts`, self-skips with no `DATABASE_URL`).
- **Frozen guardrails held:** no ingest behavior changed (comment + separator-representation + one strengthened test assertion only); advisory-only preserved (movement evidence, never custody/authority; directional emission deliberately deferred); every query clinic-scoped; no new transport; zero runtime deps; single-source `RfidBatchSchema`. Scope stayed within `packages/rfid-controller/**` — no server route or middleware touched in this re-attempt.

**Verdict:** VERIFIED

## 2026-07-17 — R-RFID INT-2 · Re-enable Module 3 serialization + align Module 7 + park the e2e

**Claim:** On `claude/rfid-gates-board-alerts` (R-M1 exports `RfidBatchSchema` WITH the R-M1.2a directional fields): (a) Module 3/5 — the controller was built internal-evidence-only (directional fields validated for parity but never serialized, a deliberate INT-1 deferral). Now the exported schema ACCEPTS them, so ENABLE the envelope builder to serialize `direction`/`fromGateway`/`toGateway` when a resolved crossing has them, staying advisory-only. (b) Module 7 — confirm the controller signs with its ONE current secret and hot-swaps on rotation; grace is SERVER-SIDE (`getRfidVerificationSecrets` → `[current, previous]`); fix the stale test comment that assumed no server grace existed, and assert the server-grace property in the DB-integration e2e. (c) e2e — confirm isolated random-id clinics + best-effort purge + self-skip without `DATABASE_URL`; NOT run against the shared dev DB in this card.

**Evidence:**
- **(a) RED→GREEN, directional serialization (added behavior):** rewrote `packages/rfid-controller/tests/envelope.test.ts` FIRST — replaced the INT-1 "emits ONLY {tagEpc,gatewayCode,readAt} — directional emission deferred" test with (i) a first-sighting test (fromGateway === null → minimal triple, both-or-neither holds), (ii) a resolved-crossing test asserting `fromGateway`/`toGateway` serialize with `toGateway === gatewayCode` (destination), (iii) a classified-direction test, and (iv) a validation test importing the REAL exported `RfidBatchSchema` (`vi.mock("../../../server/db.js", () => ({ db: {} }))` like `contract-parity.test.ts`) asserting the directional batch `safeParse().success === true` and the signed body bytes equal the validated batch. Ran → RED for the right reason: `expected undefined to be 'GW-1'` (fromGateway) and `expected undefined to be 'entered'` (direction), `2 failed | 124 passed`. Then GREEN: added optional `direction?: RfidDirection` to `MovementEvent` (`src/aggregate.ts`) and updated `toWireEvent` (`src/envelope.ts`) to emit the gateway pair when `fromGateway !== null` and `direction` when defined → `pnpm test:rfid-controller` = `14 passed | 1 skipped`, `126 passed | 6 skipped`. Headers on `direction.ts` + `envelope.ts` rewritten from "deliberate deferral" to "additive movement evidence"; advisory-only preserved (the time/sequence tracker never fabricates entered/exited — it is carried only when an upstream hardware source supplies it).
- **(b) Module 7 — controller behavior already correct; stale comment fixed + server-grace asserted:** `src/secret-source.ts` already signs with ONE `current()` secret and hot-swaps via `RotatableSecretSource.rotate()` (previous retained for observability only, never signed with) — no code change needed. Fixed the false comment in `tests/secret-source.test.ts` ("No server-side current-OR-previous grace verifier exists on this branch") to state the truth: the controller never dual-signs; rotation overlap is a SERVER property (`getRfidVerificationSecrets` returns `[current, previous]` during grace, ingest tries each), asserted in the e2e. Added an e2e case (`tests/e2e.test.ts`): a dedicated grace clinic (random id) seeded with `webhook_secret`=CURRENT + `previous_webhook_secret`=PREVIOUS and an open `vt_rfid_secret_rotations` grace row (`status:"grace"`, `previousRetained:true`, `graceExpiresAt`=+1h); asserts a batch signed with the PREVIOUS secret is accepted (202), the CURRENT secret is accepted, and a neither-current-nor-previous secret is dropped (401). Dedicated clinic so the `[current,previous]` set never leaks into the single-secret clinics.
- **(c) e2e isolation — confirmed:** `e2e.test.ts` uses `describe.skipIf(!HAS_DB)` (self-skips with no `DATABASE_URL`; all server-db imports are dynamic inside `beforeAll`, so nothing connects on skip), seeds per-run random-UUID clinic ids (`e2e-rfid-*`, `e2e-rfid-off-*`, new `e2e-rfid-grace-*`) — never the dev clinic — and best-effort purges only rows it created (equipment/rooms/serverConfig/credentials + the new grace rotation row + grace flag/credentials); it never deletes a clinic or append-only audit rows. NOT executed against a DB in this card (no `DATABASE_URL` set); its 6 tests self-skip.
- **Commands (final):**
  - `pnpm typecheck` (frontend `tsc --noEmit` + server `tsconfig.server.json --noEmit`) → exit 0, zero errors.
  - `pnpm rfid-controller:typecheck` (`tsc --noEmit -p packages/rfid-controller/tsconfig.json`, types the new `rfidSecretRotations` e2e insert + `RfidBatchSchema` import) → exit 0.
  - `pnpm test:rfid-controller` → `Test Files 14 passed | 1 skipped (15)`, `Tests 126 passed | 6 skipped (132)` (skip = DB-integration `e2e.test.ts`, self-skips with no `DATABASE_URL`; directional serialization covered at UNIT level in `envelope.test.ts`).
  - `pnpm test -- rfid` → `Test Files 24 passed (24)`, `Tests 135 passed (135)`.
...
## 2026-07-18 — FS-1 re-attempt: stranded-`finalizing` liveness fix (idempotent reclaim of a crash-stranded finalize)

**Claim:** Addressed the reviewer's HIGH + MEDIUM on the first FS-1 cut. HIGH: a HARD process crash (SIGKILL/OOM/Railway deploy restart) between the Phase-1 `grace`→`finalizing` CAS and the Phase-3 commit stranded the row `finalizing`/`previous_retained=true`, permanently holding the one-in-flight gate (`UNIQUE (clinic_id) WHERE previous_retained=true`) — `finalizeRotation`'s Phase-1 CAS matched ONLY `status='grace'`, so every re-entry (incl. the `getRfidVerificationSecrets` grace-expiry lazy path) no-op'd `false` and NOTHING re-drove it → `rotateRfidSecret` bricked forever (strictly worse than pre-FS-1 recoverability, where a crash-after-CAS released the gate). MEDIUM: the stated finalizing-recovery intent (comment + docs) was a silent no-op for that reason. Fixed via fix-direction (a): widen the Phase-1 CAS to reclaim a **stale** `finalizing` row and make the lazy ingest path re-drive it. Guardrails preserved: revert-on-credential-failure recoverability intact; no actively in-flight finalize is stomped; every rotation query clinic-scoped; RFID advisory-only; scan-only golden path + no-rotation ingest fast-path byte-for-byte unchanged.

**Evidence (verified against real code + DB + tests):**
- `server/lib/rfid/provisioning.ts` `finalizeRotation` — Phase-1 CAS widened from `status='grace' AND previous_retained=true` to `previous_retained=true AND (status='grace' OR (status='finalizing' AND updated_at <= now − FINALIZING_STALE_MS))` (drizzle `or(eq(status,'grace'), and(eq(status,'finalizing'), lte(updatedAt, staleBefore)))`). The CAS `SET updated_at=now` re-stamp serializes concurrent reclaimers (first winner freshens → losers' stale predicate fails → 0 rows → `false`), so an actively in-flight finalize is never double-run. New const `FINALIZING_STALE_MS = 60_000` (a real finalize completes in ms; 60s can only be a crash-stranded claim).
- `server/lib/rfid/provisioning.ts` `getRfidVerificationSecrets` — now re-drives when `graceExpired || finalizingStranded` (`status==='finalizing' && now − updated_at >= FINALIZING_STALE_MS`), not just on grace-expiry. Because finalize's CAS now reclaims a stale `finalizing`, this lazy path actually COMPLETES a crash-stranded finalize and releases the gate (previously a silent no-op). A non-stale (actively in-flight) `finalizing` row is still returned as `[current, previous]` — unchanged.
- `server/lib/metrics.ts` — added bounded closed-union counter `rfid_secret_rotation_reclaimed` (union + `DEFAULT_COUNTERS`); fired when the lazy path reclaims a stranded finalize (grace-expiry path still fires `rfid_secret_grace_expired`). No free-form labels (guardrail 6).
- `tests/rfid-provisioning.test.ts` — new DB-integration case "FS-1 crash-recovery: a stranded `finalizing` row … is reclaimed and the one-in-flight gate is released — rotation is NOT bricked": stages a stranded `finalizing`/`previous_retained=true` row with a 10-min-old `updated_at` and a still-open (600s) grace window (so ONLY staleness — not grace-expiry — can reclaim), asserts a fresh rotate first trips `ROTATION_IN_PROGRESS` (gate held), then `getRfidVerificationSecrets` reclaims it (`[current]` only), the row is durably `completed` + `previous_retained=false`, and a fresh rotate then SUCCEEDS (grace). RED first: with the source fix stashed (old code) the same test fails `expected [ …(2) ] to deeply equal [ Array(1) ]` (stranded row never reclaimed — the exact defect). Non-flaky (3× standalone → 1 passed each).

**Evidence — commands:**
- `pnpm typecheck` (frontend `tsc --noEmit` + server `tsconfig.server.json --noEmit`) → exit 0, zero errors.
- RED: `git stash push server/lib/rfid/provisioning.ts` then `npx vitest run tests/rfid-provisioning.test.ts -t "crash-recovery"` → `Tests 1 failed | 18 skipped (19)` (`expected [ …(2) ] to deeply equal [ Array(1) ]`); `git stash pop`.
- GREEN: `DATABASE_URL=… npx vitest run tests/rfid-provisioning.test.ts` → `Test Files 1 passed (1)`, `Tests 19 passed (19)` (18 prior FS-1/rotation + 1 new crash-recovery; DB cases run, not skipped).
- Broad regression: `npx vitest run` (clean env; `dotenv/config` still loads DATABASE_URL so DB-integration cases run) → `Test Files 635 passed (635)`, `Tests 5702 passed | 11 skipped (5713)`, zero failures.

**Guardrails held:** recoverability intact + strengthened (revert-on-delete-failure unchanged; crash-stranded finalize now self-heals instead of bricking the gate); no actively in-flight finalize stomped (60s staleness gate + CAS re-stamp mutual exclusion); the transient-`completed` window stays closed (`completed` still reached only after the durable delete); all rotation queries clinic-scoped; RFID advisory-only (no custody writes); scan-only golden path + no-rotation ingest fast-path byte-for-byte unchanged; no new migration (176 already additive+idempotent, applied + verified live); no transport/authority change.

**Verdict:** VERIFIED

## 2026-07-18 — FS-1 re-attempt round 2: scheduled backstop for crash-stranded `finalizing` (POST-delete window + quiet-clinic)

**Claim:** Addressed the reviewer's HIGH + MEDIUM + LOW on the round-1 FS-1 liveness fix. HIGH: the lazy-only reclaim (`getRfidVerificationSecrets` re-driving a stale `finalizing` row) closes only the PRE-delete sub-window — a crash AFTER Phase-2 durably removed `previous` from the credential blob but BEFORE the Phase-3 status CAS strands the row `finalizing`/`previous_retained=true` while the blob no longer carries `previous`, so `getRfidVerificationSecrets` short-circuits at its `!previous` early return (the frozen no-extra-query common path) and NEVER re-drives finalize → one-in-flight gate bricked forever. MEDIUM: even the pre-delete window depends on CONTINUED ingest traffic, so a clinic whose readers fall quiet right after stranding holds the gate with no upper bound. LOW: the round-1 crash test left `previous_webhook_secret` in the blob (only the pre-delete sub-window). Fixed with a scheduled sweeper that re-drives every STALE `finalizing` row through the same two-phase finalize regardless of blob state — closing the HIGH post-delete window and the MEDIUM quiet-clinic case uniformly WITHOUT touching the frozen hot ingest path.

**Evidence (verified against real code + DB + tests):**
- `server/lib/rfid/provisioning.ts` — new exported `reclaimStrandedFinalizingRotations(now?)`: SELECTs `status='finalizing' AND previous_retained=true AND updated_at <= now − FINALIZING_STALE_MS` (cross-clinic staleness scan, mirroring `runRfidReaderOfflineSweep`) and re-drives each through the clinic-scoped `finalizeRotation` (Phase-1 CAS already accepts a stale `finalizing`; Phase-2 delete is an idempotent no-op when `previous` is already gone; Phase-3 CAS → `completed`). Fires the existing bounded counter `rfid_secret_rotation_reclaimed` per reclaimed row; per-row try/catch so one clinic's transient failure never aborts the sweep. No new metric (reuses the round-1 closed-union counter).
- `server/lib/rfid/finalizing-sweep.ts` (new) — `startRfidFinalizingSweep()`: fixed 60s cadence + startup sweep, single-start guard, mirroring `reader-offline-sweep.ts`. Releases a stranded gate within one interval even with zero ingest/ack traffic.
- `server/app/start-schedulers.ts` — imports + registers `startRfidFinalizingSweep()` alongside `startRfidReaderOfflineSweep()`.
- `tests/rfid-provisioning.test.ts` — new DB-integration case "FS-1 crash-recovery (POST-delete)": stages a stranded `finalizing`/`previous_retained=true` row with `updated_at` 10-min-old AND strips `previous_webhook_secret` from the blob (`storeCredentials(clinic,"rfid",{webhook_secret})`), asserts the LAZY ingest path leaves it stranded (`getRfidVerificationSecrets → [current]`, row still `finalizing`, fresh rotate trips `ROTATION_IN_PROGRESS`), then `reclaimStrandedFinalizingRotations()` reclaims it (`reclaimed ≥ 1`), the row is durably `completed`/`previous_retained=false`, and a fresh rotate SUCCEEDS (grace). Imports `reclaimStrandedFinalizingRotations` + `storeCredentials` (mocked module delegates to the real store when no finalize hook armed).
- `docs/audit/rfid-rotation-finalizing-state-backlog.md` — appended "RE-ATTEMPT ROUND 2" note (stays RESOLVED, kept for provenance).

**Evidence — commands:**
- `pnpm typecheck` (frontend `tsc --noEmit` + server `tsconfig.server.json --noEmit`) → exit 0, zero errors.
- RED: injected `return {scanned:0,reclaimed:0}` at the top of `reclaimStrandedFinalizingRotations` (simulates "no backstop") → `npx vitest run tests/rfid-provisioning.test.ts -t "POST-delete"` → `Tests 1 failed | 19 skipped` (`AssertionError: expected 0 to be greater than or equal to 1` at the `swept.reclaimed` assertion — the earlier assertions that the lazy path leaves the row stranded + gate held PASSED, isolating the sweep as the fix). Probe removed.
- GREEN: `DATABASE_URL=… npx vitest run tests/rfid-provisioning.test.ts` → `Test Files 1 passed (1)`, `Tests 20 passed (20)` (19 prior FS-1/rotation + 1 new POST-delete; DB cases run, not skipped).
- Broad regression: `npx vitest run` (clean env; `dotenv/config` still loads DATABASE_URL so DB-integration cases run) → `Test Files 635 passed (635)`, `Tests 5703 passed | 11 skipped (5714)`, zero failures.

**Guardrails held:** the transient-`completed` window stays closed (`completed` reached only after the durable delete); recoverability intact (Phase-2 delete-failure still reverts `finalizing`→`grace`); no actively in-flight finalize stomped (60s staleness gate + CAS re-stamp mutual exclusion, unchanged); every rotation query clinic-scoped (the staleness SELECT is a system sweep like reader-offline-sweep, each reclaim clinic-scoped via `finalizeRotation`); RFID advisory-only (no custody writes); scan-only golden path + no-rotation ingest fast-path byte-for-byte unchanged (the fix is a scheduled sweep, NOT a new query on the hot `!previous` path); no new migration (176 already additive+idempotent); no transport/authority change.

**Verdict:** VERIFIED

## 2026-07-18 — FS-1 re-attempt round 3: revert-stomp ownership CAS (HIGH) + Phase-3 winner-only return (MEDIUM) + 3 CodeRabbit quick-wins

**Claim:** Fixed the reviewer's HIGH revert-stomp race that the round-1/2 Phase-1 widening introduced, plus a MEDIUM double-count, plus 3 CodeRabbit quick-wins — all on the finalizing delta, one commit. HIGH: `revertRotationToGrace` did an UNCONDITIONAL `UPDATE … SET status='grace', previous_retained=true, completed_at=null WHERE clinic_id AND id` (no status/updated_at predicate). Safe pre-delta (one path per finalizing row), but round-1 WIDENED `finalizeRotation` Phase-1 to also reclaim an already-`finalizing` row once stale, so TWO paths can act on one finalizing row. FAILURE: (1) Process A claims grace→finalizing at t0; (2) A's Phase-2 `storeCredentials` sits SLOW (>60s, not crashed); (3) the sweep re-claims the stale row at t65 and completes it (delete ok, status=completed, previous_retained=false, gate released — correct); (4) at t90 A's slow store finally FAILS → A's catch runs `revertRotationToGrace(); throw`; (5) the unguarded revert STOMPS the `completed` row back to `grace`/`previous_retained=true` with NO `previous` in the blob → row stuck forever (`getRfidVerificationSecrets` short-circuits on `!previous`; `reclaimStrandedFinalizingRotations` selects only `finalizing`) → one-in-flight gate held forever, clinic can never rotate again. MEDIUM: Phase-3 did a blanket `UPDATE … WHERE status='finalizing'` and unconditionally `return true`, so a loser (whose claim a concurrent reclaimer superseded) also returned `true`, double-counting `rfid_secret_rotation_reclaimed` and violating the documented "returns whether THIS call drove it to `completed`" contract.

**Evidence (verified against real code + DB + tests):**
- `server/lib/rfid/provisioning.ts` `revertRotationToGrace` — signature widened to `(clinicId, rotationId, fromStatus: RotationStatus, claimedUpdatedAt: Date, now)`; the WHERE now adds `eq(status, fromStatus)` + `eq(updatedAt, claimedUpdatedAt)`. Reverts ONLY the exact row THIS caller claimed; if a concurrent reclaimer re-claimed (updated_at changed) or completed (status changed) the row, the CAS matches 0 rows → no stomp. Legitimate non-raced revert (caller still owns the row) still matches → grace, previous still retained.
- `server/lib/rfid/provisioning.ts` `finalizeRotation` — Phase-1 `.returning()` row captured as `claimedRow`; the exact claimed `updated_at` threaded into BOTH the Phase-2 delete-failure revert (`fromStatus='finalizing'`, `claimedRow.updatedAt`) AND the Phase-3 commit (`.where(… eq(status,'finalizing'), eq(updatedAt, claimedRow.updatedAt))` + `.returning()`; `return completed.length > 0`). Only the call still owning the claim drives `completed`/returns `true`; a superseded loser matches 0 rows → `false` → the reclaim metric increments exactly once.
- `server/lib/rfid/provisioning.ts` `rollbackRfidSecret` (other call site) — its restore-failure revert now passes `fromStatus='rolled_back'` + the rollback CAS's own `claimed.updatedAt`. No concurrent path touches a `rolled_back` row, so this preserves the existing rollback-revert behavior exactly (belt-and-suspenders ownership) — rollback keeps working.
- `server/lib/rfid/provisioning.ts` — new `export const __test = { finalizeRotation, revertRotationToGrace }` (test-only; never imported by production) so the DB tests can drive the ownership boundary directly.
- `migrations/176_vt_rfid_secret_rotations_finalizing_status.sql` (CodeRabbit 3607058085) — removed the DO-block guard that checked `conname` GLOBALLY (would suppress the ADD if a same-named constraint existed on ANY unrelated table). The preceding unconditional `DROP CONSTRAINT IF EXISTS` already clears it, so the ADD is now an unconditional `ALTER TABLE … ADD CONSTRAINT`; DROP-then-ADD stays additive+idempotent. Verified live: applied twice back-to-back → both ok; constraint def = `CHECK (status IN ('grace','finalizing','completed','rolled_back'))`.
- `tests/rfid-provisioning.test.ts` (CodeRabbit 3607058092) — added a validated `dbPool()` accessor (null-checks once, throws otherwise); replaced every `probePool!` non-null assertion with `dbPool()`. Replaced the three `let release!:…` definite-assignment gates with a no-`!` `makeGate()` helper (no-op default reassigned synchronously in the executor).
- `tests/rfid-provisioning.test.ts` (CodeRabbit 3607058093) — wrapped each gated/blocked credential phase (the three `beforeFinalizeDelete`-parked tests) in `try { … } finally { release(); await ackPromise.catch(()=>{}); }` so a failing assertion always releases the parked hook + settles the promise (no hung teardown).
- `tests/rfid-provisioning.test.ts` — 3 new DB-integration cases: (a) HIGH regression — a stale `finalizing` row driven to `completed` by the sweep, then the losing finalize's late `revertRotationToGrace(…, 'finalizing', staleClaimedUpdatedAt, …)` must leave it `completed` (NOT stomped to grace) and a fresh rotate SUCCEEDS (gate free); (b) a FRESH (non-stale) `finalizing` row is left untouched by `reclaimStrandedFinalizingRotations` (in-flight finalize never stomped); (c) two concurrent `finalizeRotation` on the SAME stale row → exactly ONE returns `true`/drives `completed`, the loser returns `false` with no throw.

**Evidence — commands (test ran for real against local Postgres; `DATABASE_URL` from `.env`, DB reachable — cases ran, not skipped):**
- `pnpm typecheck` (frontend `tsc --noEmit` + server `tsconfig.server.json --noEmit`) → exit 0, zero errors.
- RED (non-tautological): temporarily stripped the `eq(status, fromStatus)` + `eq(updatedAt, claimedUpdatedAt)` predicates from `revertRotationToGrace` (the pre-fix unguarded revert) → `pnpm test -- tests/rfid-provisioning.test.ts -t "losing finalize"` → `Tests 1 failed | 22 skipped` with `AssertionError: expected 'grace' to be 'completed'` at the "NOT stomped back to grace" assert (proves the exact HIGH stomp). Restored the guard.
- GREEN: `pnpm test -- tests/rfid-provisioning.test.ts` → `Test Files 1 passed (1)`, `Tests 23 passed (23)` (20 prior + 3 new; DB cases ran, not skipped).
- Migration idempotency: applied the reworked 176 SQL twice via `pg.Pool` → `apply #1 ok` / `apply #2 ok (idempotent)`; constraint def verified.
...
## 2026-07-21 — Doctor-Pilot Unblock: actOffShift capability, NFC confirm, board custody (8-task TDD plan, branch claude/doctor-pilot-client-board-8b86d6)

**Claim:** Executed the 8-task doctor-pilot unblock plan on worktree `/Users/dan/vettrack/.claude/worktrees/rfid-vendor-agnostic-core-884f58`, branch `claude/doctor-pilot-client-board-8b86d6`. Task 0 rebase onto `origin/main`; Tasks 1-8 each TDD (RED confirmed before implementation, GREEN after), committed individually. Final verification green.

**Evidence (verified against real code + tests this session):**
- **Task 0 (rebase, no commit):** `git rebase origin/main` — the branch's only commit (`aad37b75e`, a docs-only audit report) dropped automatically by git as "patch contents already upstream" (its content was already squash-merged into `origin/main`'s `a428cba42`, verified via `git diff aad37b75e a428cba42 -- docs/audit/pre-resubmission-4flow-audit-2026-07-18.md` → empty diff). Post-rebase `HEAD` == `origin/main` exactly (`git log --oneline -3` confirms `a428cba42` at HEAD). Environment gap found+fixed: no `vettrack_test` Postgres DB existed in this worktree (`pnpm test` failed 20 DB-integration tests with `database "vettrack_test" does not exist`) — created it (`CREATE DATABASE vettrack_test OWNER vettrack`) and ran `pnpm db:migrate` against it (177 migrations applied clean). After that, `pnpm typecheck` → 0 errors; `pnpm test` → `Test Files 649 passed (649)`, `Tests 5839 passed | 11 skipped (5850)`.
- **Task 1 (`748f765c6`):** `src/lib/roles/experience-model.ts` — new `Capability` member `equipment.actOffShift`; granted to `vet` archetype, admin archetype, and `SECONDARY_ADMIN_CAPS`; added to `WITHHELD_FROM_STUDENT`. Not added to `SHIFT_SENSITIVE` or `TECH_CAPABILITIES` (verified via `grep -n actOffShift` — 5 occurrences, matching exactly the union + 4 grant/withhold sites). RED: `pnpm test -- tests/experience-model.test.ts` → 2 failed (vet/admin grant assertions) before implementation. Deviation: a pre-existing tripwire test in the same file (`per-archetype capability snapshots`, line ~309) hard-lists each archetype's exact capability set and failed after the deliberate grant (admin/vet snapshots missing the new capability) — updated the SNAPSHOT literals to include `equipment.actOffShift` for admin/vet (the test's own docstring: "A change here must be a deliberate edit"). GREEN: `pnpm test -- tests/experience-model.test.ts` → 33 passed. `pnpm typecheck` → 0.
- **Task 2 (`57710df94`):** new `src/lib/shift-gate.ts` (`shouldBlockForShift`) + `tests/shift-gate.test.ts`. RED: module-not-found error before the file existed. GREEN: 4 passed.
- **Task 3 (`b9b6f2fd5`):** wired `shouldBlockForShift` + `useExperience().can("equipment.actOffShift")` into `ScanScreen.tsx`, `EquipmentActions.tsx` (`handleCheckout`), `EquipmentDetailScreen.tsx` (`handleReservationCheckout`); updated `tests/helpers/equipment-actions-mocks.ts` `AuthValue`/`DEFAULT_AUTH_VALUE` with role/effectiveRole/roleSource. RED: `pnpm test -- tests/scan-screen-admin-shift-bypass.test.tsx` → 2 new cases (vet-no-shift, shift-error-defers) failed, 5 pre-existing passed, matching plan. Three deviations found+fixed to preserve pre-existing test intent against the new default-admin-exempt behavior: (a) `ScanScreen.tsx` render ternary reordered to `shiftLoading ? null : scanBlocked ? …` — the literal plan snippet (`scanBlocked` with no loading term, ternary order unchanged) broke the pre-existing "scanner does not mount while shift query is pending" case, since `scanBlocked` no longer includes a `!shiftLoading` term; (b) `tests/equipment-actions.test.tsx`'s "blocks checkout off-shift" test used the mock's default admin authState, which now legitimately bypasses the gate (Task 1's intent) — repointed that test to a non-exempt technician role and added a new "admin off-shift checkout is exempt" case; (c) `tests/phase-7-dead-end-fixes.test.ts` asserted the literal old source string `if (!hasActiveShift)` in `EquipmentDetailScreen.tsx` — updated to assert `shouldBlockForShift({` (the gate still exists, expressed differently). GREEN: all three test files pass (29 tests); full suite `pnpm test` → 650 files / 5850 passed / 11 skipped. `pnpm typecheck` → 0.
- **Task 4 (`39c34d9b5`):** `EquipmentActions.tsx` `canCheckout` now allows Take from `returned` custody when `!equipment.homeRoomId`. RED: 2 of 4 new cases failed as the plan predicted. Deviations found+fixed: a pre-existing test ("does not show Checkout for a returned item") used the `dockedAvailable` fixture which has no `homeRoomId` set (undefined → falsy → now correctly SHOWS checkout under the new logic) — added `homeRoomId: "room-1"` to that fixture so it tests the intended dock-return-routing case; `tests/stage-6-equipment-detail-token-consistency.test.js` asserted the literal old `canCheckout` source string — updated to the new multi-line literal. GREEN: `tests/equipment-actions.test.tsx` → 24 passed (5 new added: returned/no-dock shows, returned/with-dock hides, docked/with-dock unchanged, vet-off-shift-fires); `stage-6-…` → 19 passed; full suite → 650/5854 passed. `pnpm typecheck` → 0.
- **Task 5 (`86e2dc51e`):** `EquipmentDetailScreen.tsx` — report-issue `Button` (`data-testid="btn-detail-report-issue"`, `Flag` icon, `t.qrScanner.reportIssue`) added before `ReportEquipmentIssueSheet`. No dedicated test in this task per plan (covered by Task 6); full suite stayed green (650/5854) and `pnpm typecheck` → 0.
- **Task 6 (`45b706e09`):** locale keys `equipmentNfc.{confirmTakeTitle,confirmReturnTitle,confirmTake,confirmReturn}` added to both `en.json`/`he.json` + hand-built `src/lib/i18n.ts` accessor; new `AlertDialog` in `EquipmentDetailScreen.tsx` consuming `?nfcAction=toggle` (dismiss `"nfc-open"` toast, strip params, arm `nfcPending`, guard on held-by-other via `toggleBlocked`, confirm fires `runEquipmentQuickToggle` + `markNfcToggleFired`). New `tests/equipment-detail-screen-nfc-confirm.test.tsx` (6 cases: Take dialog, confirm fires toggle, cancel closes, held-by-other blocks with no dialog, held-by-me shows Return dialog, no-params shows report-issue + opens issue sheet). RED confirmed (5/6 failed pre-implementation, the no-op "no NFC params" case passed as expected). GREEN on first implementation attempt (no plan deviation needed) — 6/6 passed. `pnpm typecheck` → 0; `pnpm i18n:check` → parity; `pnpm test -- tests/i18n-parity.test.ts tests/i18n-no-hebrew-in-source.test.ts` → 6 passed; full suite → 651/5860 passed.
- **Task 7 (`4910715b6`):** precondition grep (`isCapacitorNative` in `EquipmentDetailToolsSheet.tsx`) confirmed NO prior match, so the task proceeded (not skipped). New `tests/equipment-detail-tools-sheet-native.test.tsx` (native=false shows Print QR, native=true hides it). RED: native=true case failed pre-implementation. `EquipmentDetailToolsSheet.tsx` Print QR `Button` wrapped in `{!isCapacitorNative() && (…)}`. GREEN: 2/2 passed; full suite → 652/5862 passed. `pnpm typecheck` → 0.
- **Task 8 (`2d6967e2f`):** `shared/equipment-board.ts` new `EquipmentBoardCustodyBlock` type + `custody?` on `EquipmentCommandBoardSnapshot`; `server/services/equipment-command-board.service.ts` new `queryCustody()` (bounded `CUSTODY_BLOCK_LIMIT=12`, `clinicId`-filtered, `custodyState='checked_out'`, skips null-email rows), wired into `BoardAggregateFns`/`defaultBoardAggregates`/`Promise.all`/snapshot return; `criticalUnits` rows now populate `custodianName` from `checkedOutByEmail` when `custodyState==='checked_out'`. `locales/{en,he}.json` `board.custody` key added (passthrough accessor, `board: d.board`, no `i18n.ts` edit needed — verified). `src/features/command-board/components/board-panels.tsx` new `CustodyPanel`; `CommandBoard.tsx` mounts it guarded on `board.custody && board.custody.units.length > 0`. `tests/command-board-aggregates-degradation.test.ts` extended: function-presence loop now asserts 5 keys (added `custody`); new degradation case (`safeBlock` synthetic throw, DB-free per the file's existing pattern). RED: the function-presence assertion failed pre-implementation (`typeof undefined` !== `"function"`). GREEN: 6/6 passed. `pnpm typecheck` → 0; full suite → 652/5863 passed.

**Final verification (after all 8 tasks, from repo root of the worktree):**
- `pnpm typecheck` → exit 0, zero errors (both tsconfigs).
- `pnpm test` → `Test Files 652 passed (652)`, `Tests 5863 passed | 11 skipped (5874)`, 0 failed.
- `pnpm i18n:check` → `✓ locales/en.json and locales/he.json are in deep key parity.`
- `pnpm architecture:gates` → `All G1 checks passed` (TypeScript frontend+server, dependency-cruiser 939 modules/4835 deps/0 new violations beyond the 10 pre-existing known-ignored, madge cycle baseline: server 2/src 0, matches baseline).
- `pnpm tenant:lint:touched` (warn-only) → `WARN — 7 possible missing clinicId scope`, all inside `server/services/equipment-command-board.service.ts`, including the new `queryCustody` (line 365, `.from(equipment)`) — verified as a FALSE POSITIVE: `queryCustody` correctly filters `eq(equipment.clinicId, clinicId)` in its `.where()` two lines below the flagged line (read directly). Confirmed this is a systemic heuristic gap on this file's multi-line `.where(and(eq(x.clinicId, clinicId), …))` pattern, not something Task 8 introduced: the other 6 flagged lines (`queryDocks` line 320, `queryPower`/`queryWaitlist`/`queryStaging`, `rfidEgressSignals`, `equipmentRfidReads`) are pre-existing functions this task did not touch, each independently confirmed (spot-checked `queryDocks` at line 320) to already filter `clinicId` in the same chained-`.where()` shape — the tool's warn-only file-level scan surfaces all of them together whenever the file is touched. Not part of `architecture:gates`; does not block.

**Commits (branch `claude/doctor-pilot-client-board-8b86d6`, all ahead of `origin/main`, none pushed, no PR opened):**
1. `748f765c6` feat(roles): equipment.actOffShift capability for vet + admin (doctor pilot)
2. `57710df94` feat(shift-gate): shouldBlockForShift helper reading the actOffShift capability
3. `b9b6f2fd5` feat(shift-gate): wire actOffShift capability into scan, checkout, and reservation claim
4. `39c34d9b5` fix(equipment): allow Take from 'returned' custody when there is no home dock (C5)
5. `86e2dc51e` feat(equipment): report-issue button on the mobile record (C7)
6. `45b706e09` feat(nfc): mobile record consumes the NFC deep-link with an explicit Take/Return confirm (C4)
7. `4910715b6` fix(native): hide Print QR in the equipment tools sheet on the Capacitor shell (IPHONE-2)
...
## 2026-07-22 — Addressed CodeRabbit's 15 actionable comments on PR #133 (branch docs/2.0-planning-corpus)

**Claim:** All 15 CodeRabbit comments on PR #133 investigated against actual file content (per pr-rules.md — no comment is discretionary); all 15 confirmed real and fixed; none skipped.

**Evidence (verified this session):**
- `gh pr view 133 --json ...` (via the background polling script) → CI/Playwright/Vercel all `SUCCESS`, `mergeStateStatus: CLEAN`, but `reviewDecision: CHANGES_REQUESTED` with a CodeRabbit review posting 15 actionable inline comments across 9 files.
- **Real React bugs found and fixed in Task 5/7 of `docs/plans/master-plan-2026-07.md`** (verified by re-reading the actual drafted code, not assumed from the comment text alone): (1) `RoomPresenceSubscriber` called `onUpdate` during render instead of an effect — fixed with `useEffect` keyed on `[roomId, presentMembers]`. (2) `useFloorPresence`'s internal `handleUpdate` was never exposed, so `FloorPresenceCard` wired subscribers to its own disconnected `updates` state — `floors` would have rendered permanently empty; fixed by having the hook return `{ floors, handleRoomUpdate }` and wiring subscribers directly to it, deleting the dead local state. (3) `isStale` was computed inside a `useMemo` keyed on `[rooms, snapshots]` only, so nothing forced recomputation once 90s passed with no new presence event — added a `tick` state driven by `setInterval` in the deps array; rewrote the test to use `vi.useFakeTimers()`/`vi.advanceTimersByTime()` (the original test's `mockNow += 91_000; rerender()` pattern would not have actually exercised the fix, since neither `rooms` nor `snapshots` change on that `rerender()` call).
- **`FloorPresenceCard` loading/error state**: confirmed the original had no handling for `api.rooms.list()` rejecting — added `status: "loading" | "error" | "ready"` with a retry button, and a second test case (`roomsListImpl` throwing) asserting the error UI renders.
- **`FloorSheet` accessible modal**: confirmed the original had an unconditional `onClick={() => onOpenChange(false)}` on the outer `div` (any click inside dismisses it) and no focus management — added `useEffect`-driven focus-in-on-open/restore-on-close, an `Escape` keydown handler, `aria-modal="true"`, and a real close button; removed the outer-div dismiss-on-click.
- **`docs/plans/2.0/task-2.3-who-on-floor.md`**: confirmed lines 52-53 promised "join time, role, you-marker" in `FloorSheet` while the presence contract (`useRecordPresence`'s `presentMembers: {userId, displayName}[]`) carries neither — trimmed the promise to match reality; extended the Verify/Execute section with explicit FloorSheet test-coverage requirements (open/close, empty state, Escape, focus restoration, inside-click-doesn't-dismiss).
- **`docs/design/vettrack-2.0-claude-design-prompt.md`**: `grep -c` confirmed the roadmap tracker is 19 items / 6 done, not the doc's stated "18" — fixed; softened "treat it as authoritative shape" (Task 1.1's uncommitted worktree schema) to "current best-known shape... not a committed contract," pointing to `packages/contracts` as the actual stable alternative.
- **`docs/vettrack-2.0-roadmap.md`**: confirmed line 471's "Shell strategy confirmed — Capacitor stays" is real and now contradicted — marked superseded with a dated note and strikethrough, kept as historical context; confirmed the Android Task 1.3 "Done when" line allowed an internal-track fallback that conflicts with the master plan's Terminal State section — reconciled to require production, with internal-track reframed as a verification gate, not a completion state.
- **`.claude/rules/agent-conduct.md`**: confirmed Rules 1 and 2 read as being in tension (research vs. authorization) — added an explicit clause: read-only inspection to fulfill the literal request is implicitly authorized; side effects (commit/push/PR/merge/post) still require explicit authorization.
- **`scripts/vettrack-2.0-scope-gate.sh`**: confirmed two real gaps — `cd ... || exit 0` treated a failed directory change as a pass (silent false-green), and the gate only checked a total count (19), which a duplicated+deleted ID pair could satisfy while corrupting the tracker. Rewrote to fail closed on `cd`/read failure and to diff the actual extracted ID set against a canonical 19-ID list, catching duplicates and substitutions. **Ran the rewritten script directly** (not just read it): green path against the real roadmap → `[2.0-gate] VetTrack 2.0 scope: 6/19 shipped.` (exit 0, matches `grep -c` ground truth); injected a duplicate `0.1` line into a working copy → `[2.0-gate] BLOCKED: duplicate task ID(s) in the tracker: 0.1` (exit 2); restored the real file from a `/tmp` backup and confirmed via `grep` that both the test injection was gone and the two real edits (supersession note, Android done-when) were intact.
- **`session-2.md` gap (owner-approved fix, not a CodeRabbit comment but the same root cause CodeRabbit's roadmap-citation comment flagged)**: traced via `git log --all --diff-filter=A -- "**/session-2.md"` → added by commit `23a04d193` on `claude/docs-cleanup` (this session's starting branch, confirmed via the session's own initial `gitStatus`), never merged to `main`; `docs/2.0-planning-corpus` was cut fresh from `origin/main`, so its lineage never included that commit — nothing was deleted. Copied via `git show origin/claude/docs-cleanup:.claude/docs/ai/vettrack/10x/session-2.md > ...` (250 lines) onto this branch, per the owner's explicit choice among the plan's own named remediation options.
- All 6 modified files re-`grep`'d after editing to confirm no stale reference to the old `useFloorPresence` bare-array return shape survived, and that all newly-introduced i18n keys (`floorPresence.loading/loadError/retry/close`) are present in both `locales/he.json` and `locales/en.json` code blocks in the master plan.

**Verdict:** VERIFIED — all 15 comments addressed; 0 skipped as false positives (all were confirmed real against actual file content before fixing).

## 2026-07-22 — Second CodeRabbit round on PR #133 (branch docs/2.0-planning-corpus)

**Claim:** Addressed CodeRabbit's second review (7 comments, incl. 1 duplicate on the first round's agent-conduct.md fix) — all confirmed real against actual file content and fixed.

**Evidence (verified this session):**
- Re-read `.claude/rules/agent-conduct.md` after my first fix and confirmed the duplicate finding was correct: my clause only covered "read-only repository inspection," leaving Rule 1's *external* research requirement (docs/search engines) still in unresolved tension with Rule 2. Extended the clause to cover both explicitly.
- Read the recovered `.claude/docs/ai/vettrack/10x/session-2.md` at the two flagged line ranges directly: line 43 ("one-tap approve/edit/reject, live over the Socket.io presence channel") and line 214-215 ("Equipment tracking, Code Blue... must continue when the network... is unavailable") — both confirmed to genuinely conflict with `docs/CLAUDE.md`'s frozen invariants (collab channel ephemeral-only/never-domain-state; Code Blue online-only/fail-loud/never-queued). Did **not** rewrite the original owner-authored prose (this is a recovered historical document, not a live spec) — added a dated editorial note at the top citing the exact two phrasings and what supersedes them, preserving the original text verbatim below it.
- Re-read the drafted `RoomPresenceSubscriber` code and confirmed the finding: the effect fired `onUpdate` with `lastUpdatedAt: Date.now()` on the very first render too, before the collab socket had actually joined the room — indistinguishable from "confirmed empty." Fixed by gating the effect on `isConnected` (already returned by `useRecordPresence` per Task 5's own documented interface).
- Re-read `FloorSheet`'s dialog markup and confirmed the finding: my first fix removed the outer `onClick` entirely rather than separating backdrop-dismiss from content-click-is-inert — outside-click-to-dismiss regressed. Fixed with a backdrop wrapper (`onClick={() => onOpenChange(false)}`) around the dialog content (`onClick={(e) => e.stopPropagation()}`).
- Re-read `FloorPresenceCard.test.tsx`'s error-state test and confirmed the finding: mocking `RoomPresenceSubscriber` as `() => null` meant no test actually exercised the Task 5/7 wiring fix end-to-end. Rewrote the mock to capture each room's `onUpdate` callback and added a third test that fires it directly and asserts the resulting avatar renders through `FloorPresenceCard`'s real render path.
- Re-read `docs/vettrack-2.0-roadmap.md`'s supersession note against Task 1.3 (Android/Capacitor ship) and confirmed the finding: the note read as if Capacitor were already invalid, contradicting Task 1.3's own Capacitor-specific steps. Reconciled: Capacitor is explicitly framed as the near-term bridge (Layer 2 precedes Layer 5 in the master plan's own sequencing) — Task 1.3 executes unchanged; only the long-term/terminal shell decision is superseded.
- Caught my own syntax error before committing: an inline `typeof x extends Map<string, infer F> ? F : never` conditional type in the rewritten `FloorPresenceCard` test mock — while arguably valid TS, it was needlessly convoluted; replaced with a named `PresenceUpdateFn` type alias for readability.

**Verdict:** VERIFIED — all 7 comments (6 distinct + 1 duplicate) addressed; 0 skipped.

## 2026-07-22 — Third CodeRabbit round on PR #133: retracted a false RFID finding (branch docs/2.0-planning-corpus)

**Claim:** Investigated CodeRabbit's third round (1 actionable + 2 outside-diff comments) — one of them, on Task 10's proposed RFID rate-limiter change, led to discovering that Task 10's entire premise (a live clinic-header spelling bug) is false. Retracted Task 10 in full rather than partially patching it.

**Evidence (verified this session, real command output):**
- CodeRabbit's comment: using the pre-auth `x-vettrack-clinic` header as the rate-limiter key would let one IP mint unlimited buckets, since the header is attacker-controlled before HMAC verification — a real security regression if Task 10 had ever been executed as originally written.
- Checking that claim required reading the actual current code, not just the task description: `sed -n '75,95p' server/middleware/rate-limiters.ts` → the limiter's own comment already states, verbatim, that keying by the clinic header pre-auth "would let one IP mint an unbounded number of 120/min buckets... defeating the DoS backstop," and `rfidEventLimiterKey()` already returns `` `ip:${...}` `` — IP-only, by design, already.
- Checked the supposed spelling mismatch directly: `grep -rn "vettrack-clinic\|vetrack-clinic" server/routes/rfid.ts server/middleware/rate-limiters.ts packages/rfid-controller/` → every call site (route, limiter, controller-package contract, and `tests/rfid-webhook-signature.test.ts`) already uses the correct two-"t" `x-vettrack-clinic` / `X-VetTrack-Clinic` spelling consistently.
- `server/lib/rfid/clinic-header.ts` (`readRfidClinicId()`) — read directly — is a dedicated single-source-of-truth helper whose own docstring states it exists specifically to prevent "the exact drift behind the original one-`t` bug." The bug described in `docs/plans/rfid-controller-package.plan.md` was real once and is already fixed and consolidated behind this helper; that planning doc is stale on this specific point.
- **Conclusion: Task 10 was never a real candidate — it was a stale claim relayed without independent verification, exactly the same failure mode already caught twice this session (ADR-006/M1.0, R-RTC-1's "3 CRITICAL").** Retracted the task in full (not patched), corrected the docs-scan findings section that originally introduced the claim, removed the stale row from the model/effort routing table, and added `docs/plans/rfid-controller-package.plan.md` to the master plan's stale-docs list so it isn't cited again without re-verification.
...
## 2026-07-22 — Reviewed all 24 GraphQL review threads on PR #133 before merge, found and fixed 1 real gap

**Claim:** Rather than trusting `reviewDecision`/the check-status color alone, queried the PR's actual review threads via GraphQL to confirm every finding across all 4 rounds was genuinely addressed — not just that new rounds stopped appearing. Found one real, previously-missed gap and fixed it; the other 6 "unresolved" threads were confirmed already handled (anchor drift or deliberate non-edit), not live gaps.

**Evidence:**
- `gh api graphql` query for `reviewThreads(first: 50) { isResolved isOutdated path line comments }` → 24 total threads, 7 unresolved. Read each of the 7 against current file content (not assumed from memory of having "already fixed everything"):
  1. `task-2.3-who-on-floor.md` (FloorSheet test coverage) — `isOutdated: true`, confirmed already addressed in round 1.
  2. `master-plan-2026-07.md:74` (Android release gate consistency) — anchor sits in the unchanged Terminal-State prose; the actual fix (reconciling `docs/vettrack-2.0-roadmap.md` Task 1.3's "Done when" line) was made round 2 on the *other* side of the inconsistency — confirmed via `sed -n` that both sides now agree (production-only, internal-track as a gate not a completion state).
  3. `docs/vettrack-2.0-roadmap.md:9` (missing `session-2.md` citation) — confirmed resolved: the file exists on this branch since round 1's recovery commit, so the citation is no longer false; anchor line itself was never edited (it didn't need to be), hence not marked outdated.
  4. `scripts/vettrack-2.0-scope-gate.sh` (fail-closed) — `isOutdated: true`, confirmed already addressed round 1.
  5. `.claude/docs/ai/vettrack/10x/session-2.md:232` (Code Blue online-only, CRITICAL severity) — confirmed the round-2 editorial note at the top of the file explicitly names this exact line and cites the correct CLAUDE.md invariant; the flagged line itself was deliberately left verbatim (historical-document preservation, not silently rewritten), so GitHub can't detect the fix by diffing that line — by design, not a gap.
  6. **`scripts/vettrack-2.0-scope-gate.sh:40` (real, previously missed gap):** re-read the round-2 comment text in full — it asked for "one strict parser... rejecting malformed IDs such as 1.1foo... auxiliary checkboxes excluded... regression coverage." I had only fixed fail-closed + duplicate/missing/extra-ID detection in round 1; the loose regex (`grep -oE '^- \[[ x]\] [0-9]+\.[0-9]+'` with no trailing boundary) would in fact silently accept `1.1foo` as `1.1`, and `done_count` came from an unrelated second grep that would count any auxiliary `- [x] ` line anywhere in the file. **Confirmed this was live risk, not theoretical:** `grep -c '^- \[[ x]\] '` vs. the strict-pattern count both currently equal 19 (no auxiliary lines exist today), but the parsing looseness itself was real.
  7. `.claude/rules/agent-conduct.md:21` ("grepping" misspelled "greping") — real, trivial; fixed.
- **Rewrote `scripts/vettrack-2.0-scope-gate.sh`** to derive the ID set, total, done_count, and open-items list from ONE strict-pattern (`^- \[[ x]\] [0-9]+\.[0-9]+ ` — space-terminated) match set, and added an explicit check that the strict-pattern line count equals the total-checkbox-line count (catching any malformed/auxiliary line as a hard block, not a silent miscount).
...
- `npx drizzle-kit generate` → reproduced the plan's predicted failure verbatim: `Error: Cannot find module './core.js' ... at Object.<anonymous> (/Users/dan/vettrack/server/schema/index.ts:2:15)`. Hand-authored `migrations/179_vt_action_proposal.sql` instead, cross-checked column-for-column against `server/schema/ops.ts`'s `actionProposal`/`actionProposalDecisionLog` table defs, following the `DO $$ ... EXCEPTION WHEN duplicate_object` enum pattern from `migrations/156_vt_shift_adjustments.sql`.
- Full suite green: `pnpm exec vitest run tests/autopilot/` → `Test Files 4 passed (4)`, `Tests 21 passed (21)` (action-proposal-types: 4, action-proposal-citation-validator: 3, action-proposal-service: 6, action-proposals.routes: 8).
- Single-decision invariant test asserts exactly the spike's proven shape: approve then reject on the same id → `rejects.toThrow(ActionProposalAlreadyDecidedError)` AND `writer.allDecisions()` has length 1 — confirmed passing.
- Cross-tenant negative test: a proposal staged for `clinic-a` fetched/approved with `clinic-b` → `rejects.toThrow(ActionProposalNotFoundError)`, not a distinguishable forbidden signal — confirmed passing.
- Typecheck: `pnpm typecheck` fails on 2 pre-existing errors unrelated to this change (`Cannot find module '@clerk/localizations'`, `Cannot find module 'socket.io-client'` — missing installed packages). Isolated via `git stash` / `git stash pop` before-vs-after diff on both `tsc --noEmit` (frontend) and `tsc -p tsconfig.server.json --noEmit` (server): `diff /tmp/fe-tsc-before.txt /tmp/fe-tsc-after.txt` → empty (2 lines both sides, identical); `diff /tmp/server-tsc-before.txt /tmp/server-tsc-after.txt` → the "before" run (stash leaves new untracked autopilot files in place against unmodified ops.ts/audit.ts/metrics.ts/db.ts) shows 18 errors from the new files referencing not-yet-added exports; the "after" run (full changeset restored) shows only the same 5 pre-existing baseline errors — confirms 0 new errors introduced by this slice on either tsconfig.
- No regression in shared files touched (audit.ts, metrics.ts, rate-limiters.ts): `pnpm exec vitest run tests/rate-limiters-f2.test.ts tests/f1-server-metrics.test.ts tests/phase-9-metrics-cardinality.test.ts tests/f1-metrics-route.test.ts tests/rfid-rate-limiter-key.test.ts` → `5 passed (5)`, `31 passed (31)`; `pnpm exec vitest run tests/phase-3-pr-3-8-1-audit-guard-regression.test.ts tests/phase-5-pr-5-7-1-audit-guard-regression.test.ts tests/authority-audit.test.ts tests/i18n-ux-audit-sweep.test.ts` → `4 passed (4)`, `55 passed (55)`.
- i18n-error-envelope governance: `pnpm exec vitest run tests/i18n-no-untranslated-api-error.test.ts` → `2 passed (2)` (the new route uses `apiError()` exclusively for 4xx/5xx, never a raw `res.status(...)`, so it needed no allowlist entry).
- Frozen-surface check: `git diff --stat main -- .` → 17 files changed (2 migrations/route/lib dirs new, 6 shared files modified with additive-only diffs, 5 test files new); none of `server/lib/shift-handover-scheduler.ts`, `server/lib/realtime-outbox.ts`, `server/lib/event-publisher.ts`, `server/lib/realtime-collab/*`, any Code Blue file, `public/sw.js`, or `vt_appointments`/`appointmentsPage` surfaces appear.
- No locale files touched (`locales/en.json` / `locales/he.json` absent from the diff) — reused existing generic `errors.*` keys (`errors.notFound`, `errors.conflict`, `errors.generic`), so `pnpm i18n:check` was not run (nothing to check).
- Multi-tenancy: every `DrizzleActionProposalWriter` query re-read directly — `findStaged`/`get` filter `eq(actionProposal.clinicId, clinicId)`; `stage`'s existing-row lookup filters by `clinicId`; `transition`'s UPDATE `WHERE` includes `eq(actionProposal.clinicId, clinicId)`; inserts (`stage`, `recordDecision`) take `clinicId` from caller-supplied, already-scoped input.

**Disclosed deviations from the task's literal RED-before-GREEN order:** (1) `server/lib/autopilot/action-proposal-writer.port.ts` (both `DrizzleActionProposalWriter` and `InMemoryActionProposalWriter`) was written before any dedicated writer-port test — it is exercised only indirectly through `action-proposal-service.test.ts`'s use of `InMemoryActionProposalWriter`, per the plan's own structure (no standalone writer-port test file was specified in the task's TESTS section). (2) `server/routes/action-proposals.ts` was written before `tests/autopilot/action-proposals.routes.test.ts` (the task's optional route-test branch was resolved *after* the route existed, not before) — the route test was still run and is fully green (8/8), and the plan's binding-law violation is disclosed here rather than silently omitted.

**Verdict:** VERIFIED.

## 2026-07-22 — Task 1.1 §3 `coordinator_reassign_off_roster` proposal type (branch feat/2.0-task-1.1-s3-coordinator-reassign, stacked on unmerged feat/2.0-task-1.1-s1-shared-infra)

**Claim:** Built the `coordinator_reassign_off_roster` proposal-type slice per `docs/plans/2.0/task-1.1-plan.md` §3: (A) made `action-proposal-service.ts`'s decide path atomic via a new `ActionProposalWriter.transitionAndRecord` (Drizzle impl wraps transition+decision-log-insert in one `db.transaction`; InMemory fake mirrors the semantics) — the §1-review carry-over fix; (B) `CoordinatorRosterReader` port (real + InMemory fake) detecting roster drift, read-only reuse of `resolveShiftCoordinator`; (C) pure `coordinator-reassign-composer.ts`; (D) `autopilotCoordinatorReassignWorker.ts` + additive registration in `start-schedulers.ts`; (E) citation-validator test extended with a coordinator-kind case; (F) `autopilotQueue.kinds.coordinatorReassignOffRoster.{title,summaryTemplate,proposedCandidateLabel}` in both locales. RED confirmed before every new implementation file (zero exceptions this slice — the §1 route-file violation is not repeated).

**Evidence (commands actually run this session, real output):**
- RED confirmed per file before implementing, each run against the not-yet-existing subject:
  - `pnpm exec vitest run tests/autopilot/action-proposal-service.test.ts` (new atomic-decide test added first) → `TypeError: writer.transitionAndRecord is not a function` (6 passed / 1 failed) — failed for the right reason (method didn't exist yet), not a syntax typo.
  - `pnpm exec vitest run tests/autopilot/coordinator-roster-reader.test.ts` → `Error: Cannot find module '.../coordinator-roster-reader.port.js'`.
  - `pnpm exec vitest run tests/autopilot/coordinator-reassign-composer.test.ts` → `Error: Cannot find module '.../coordinator-reassign-composer.js'`.
  - `pnpm exec vitest run tests/autopilot/autopilot-coordinator-worker.test.ts` → `Error: Cannot find module '/server/workers/autopilotCoordinatorReassignWorker.js'`.
- Full `tests/autopilot/` suite green after every file landed: `pnpm exec vitest run tests/autopilot/` → `Test Files 7 passed (7)`, `Tests 41 passed (41)` (§1's 21 baseline + 20 new: 1 atomic-decide test, 6 reader tests, 8 composer tests, 3 worker tests, 2 citation-validator coordinator-kind tests — 20 total, confirmed via `--reporter=verbose` per-test listing).
- Atomicity invariant test (`action-proposal-service.test.ts`): calls `writer.transitionAndRecord` directly — first call approves and asserts `writer.allDecisions()` has length 1; second call on the same (already-decided) proposal `rejects.toThrow(ActionProposalAlreadyDecidedError)` AND `writer.allDecisions()` still has length 1 (no orphaned decision row) AND `writer.get()` shows the proposal's status is unchanged from the first decision — confirmed passing.
- Off-roster signal formula (`coordinator-roster-reader.port.ts`) tested against all 6 branches from the plan: absent-from-candidates → flagged; present-in-candidates → not flagged; no persisted row → not flagged; stage≥3 with `coordinatorUserId === currentResponsibleUserId` → not re-flagged; stage≥3 with `coordinatorUserId !== currentResponsibleUserId` → still flagged; cross-tenant (clinic B query never sees clinic A's persisted row) → confirmed passing, all 6 green.
- Composer i18n non-hardcoded proof: `coordinator-reassign-composer.test.ts`'s locale-switch test calls the composer with `locale: "en"` and `locale: "he"` against the REAL `locales/{en,he}.json` dictionaries (no mocking) — asserts neither summary equals the raw key path (proving the key resolves) and the two locales render different text (impossible if the string were a hardcoded English literal) — confirmed passing.
- Worker idempotency: `autopilot-coordinator-worker.test.ts` — one off-roster signal → `{ scanned: 1, staged: 1 }`, exactly 1 row in `writer.findStaged(...)`; a second scan of the same `(clinicId, shiftDate)` → `staged: 0` on the returned count and `findStaged` still length 1 (the §1 unique-index-backed `stage()` idempotency, exercised end-to-end through the worker); no signal → `{ scanned: 1, staged: 0 }` — all 3 confirmed passing.
- `pnpm i18n:check` → `✓ locales/en.json and locales/he.json are in deep key parity.` (run after each locale edit).
- Typecheck delta (server): `npx tsc -p tsconfig.server.json --noEmit --pretty false` on this branch → 5 lines, all pre-existing missing-module errors (`@aws-sdk/s3-request-presigner`, `socket.io`, `@socket.io/redis-adapter` ×2 implicit-any). Isolated the base branch's own output via `git worktree add /tmp/vettrack-base feat/2.0-task-1.1-s1-shared-infra` + symlinked `node_modules` (worktrees don't get an installed node_modules) → same 5 lines. `diff /tmp/tsc-base.sorted.txt /tmp/tsc-current.sorted.txt` → empty (exit 0) — zero new server errors.
- Typecheck delta (frontend, sanity — no frontend files touched this slice): `npx tsc --noEmit --pretty false` on this branch → 2 pre-existing lines (`@clerk/localizations`, `socket.io-client`); same worktree comparison → `diff` empty (exit 0).
- Scope confirmation: `git diff --stat feat/2.0-task-1.1-s1-shared-infra..HEAD` → 14 files changed, 1050 insertions(+), 23 deletions(-) — exactly the files listed in this entry's claim, nothing else.
- Frozen-surface check: `git diff feat/2.0-task-1.1-s1-shared-infra..HEAD -- server/lib/shift-handover-scheduler.ts server/lib/realtime-outbox.ts server/lib/event-publisher.ts server/workers/sweep-escalation.worker.ts server/services/equipment-coordinator.service.ts | wc -l` → `0` — none of these files (including the two this slice explicitly must not modify: the escalation-ladder worker and the coordinator-resolution service) appear in the diff at all.
- Multi-tenancy: `DrizzleCoordinatorRosterReader.read()` — both the `shiftEquipmentCoordinator` and `shifts` queries carry an explicit `eq(*.clinicId, clinicId)` predicate (confirmed by Read of the file); `autopilotCoordinatorReassignWorker.ts`'s `findCandidateShiftDates` scans `shiftEquipmentCoordinator` unscoped by clinic (by design — it returns clinic+date pairs to iterate, each subsequently read through the clinicId-scoped reader) but every per-row read that follows is scoped.
- Untracked owner file `docs/deep-research-report (2).md` (appeared mid-session, out of this slice's scope): confirmed still `??` (untracked) in `git status --porcelain` after all 4 commits — never staged, never committed, never read/deleted.

**Deviations / notes:**
- The task brief's "one signal stages exactly one proposal" worker semantics is implemented as `staged` counting NEW stages only (checked via a `findStaged` lookup before calling `stageProposal`) — the underlying `stageProposal`/`writer.stage()` call still fires unconditionally per candidate (idempotent no-op on repeat), which means `logAudit`/`incrementMetric("autopilot_proposal_staged_total")` in the shared §1 `stageProposal` re-fire on every re-scan of an already-staged proposal. This is a pre-existing §1 shared-service characteristic (not something this slice was authorized to change) — disclosed here rather than silently accepted.
- No writer-port-level test file exists (per §1's own established pattern — `ActionProposalWriter` is exercised through `action-proposal-service.test.ts`, not a standalone port test); the new `transitionAndRecord` atomicity test follows that same established pattern rather than introducing a new one.
- No UI files this slice (`CoordinatorReassignCard` deferred to the later UI-shell slice, as instructed) — `autopilotQueue.kinds.coordinatorReassignOffRoster.*` keys exist and are consumed by the composer, but nothing renders them yet.
- Not committed: `docs/plans/2.0/task-1.1-plan.md`'s `docs/design/program-plan.md` artifact-map / roadmap-checkbox bookkeeping — out of this slice's file scope, left for the roadmap-owning session.
...
## 2026-07-22 — Task 1.1 §4 `restock_po_on_burn` proposal type (branch feat/2.0-task-1.1-s4-restock-po, off feat/2.0-task-1.1-s3-coordinator-reassign)

**Claim:** Built the `restock_po_on_burn` proposal-type slice per `docs/plans/2.0/task-1.1-plan.md` §4: (A) `RestockBurnReader` port (real + InMemory), v1 simple-threshold rule (`onHand ≤ reorderPoint`, no burn-rate projection), citing only observed `vt_container_items`/`vt_items` rows; (B) pure `restock-po-composer.ts` (top-up-to-par suggested-quantity rule, `AUTOPILOT_PO_SUPPLIER_PLACEHOLDER` constant); (C) the one kind-dispatched approve side effect among the 4 proposal kinds — `restock-po-approve-side-effect.ts` inserting real `vt_purchase_orders`/`vt_po_lines` rows atomically with the decision via a new `ActionProposalWriter.transitionAndRecord({ sideEffect })` parameter (Drizzle: same `db.transaction`; InMemory: side effect runs before any map mutation, so a throw leaves the proposal untouched); (D) `autopilotRestockBurnWorker.ts` (daily cron, additive registration); (E) per-kind edit-body Zod validation for `restock_po_on_burn` (`action-proposal-types.ts`'s `validateEditedContentForKind`, enforced in `decide()`, new `ActionProposalEditValidationError` mapped to 400 in the route); (F) citation-validator test extended with a restock-kind case, including a case proving a *derived* value (the summed `onHand` total) is correctly rejected as an ungrounded citation; (G) `autopilotQueue.kinds.restockPoOnBurn.{title,summaryTemplate,suggestedQuantityLabel}` in both locales. RED confirmed before every new implementation file with one disclosed exception (below).

**Evidence (commands actually run this session, real output):**
- RED confirmed per file before implementing, each run against the not-yet-existing subject:
  - `pnpm exec vitest run tests/autopilot/restock-burn-reader.test.ts` → `Error: Cannot find module '.../restock-burn-reader.port.js'`.
  - `pnpm exec vitest run tests/autopilot/restock-po-composer.test.ts` → `Error: Cannot find module '.../restock-po-composer.js'`.
  - `pnpm exec vitest run tests/autopilot/action-proposal-service.test.ts` (side-effect + edit-validation cases added first) → `4 failed | 10 passed` — failures were `ActionProposalEditValidationError` not exported and the mocked `buildRestockPoApproveSideEffect` module not yet existing/wired, i.e. failed for the right reason, not a typo.
  - `pnpm exec vitest run tests/autopilot/autopilot-restock-worker.test.ts` → `Error: Cannot find module '/server/workers/autopilotRestockBurnWorker.js'`.
- Disclosed RED-before-GREEN exception: `restock-po-approve-side-effect.ts` was implemented before `tests/autopilot/restock-po-approve-side-effect.test.ts` existed — it was built first to satisfy the mocked side-effect-dispatch tests added to `action-proposal-service.test.ts` (deliverable C). The dedicated pure-function test file for it was written and run against the already-existing implementation, not before — a real process deviation for this one file, disclosed rather than silently presented as RED-first. `citation-validator` union-literal RED is likewise not independently observable: the `sourceTable` union (`action-proposal-types.ts`) was extended as a shared prerequisite for the reader/composer files before the citation-validator test's new `restock_po_on_burn` block was added, so that specific test block never ran red on its own — the underlying `validateActionProposalCitations` function itself is unchanged, kind-agnostic code proven red/green in the §1 slice.
- Full `tests/autopilot/` suite green after every file landed: `pnpm exec vitest run tests/autopilot/ --reporter=verbose` → `Test Files 11 passed (11)`, `Tests 74 passed (74)` — the pre-existing 42 (§1's 21 + §3's 21) unchanged/still passing, plus 32 new: 6 citation-validator additions net (3 restock-kind cases; the file total for that suite went 3→8 lines but only 3 are net-new blocks beyond §3's prior 5 shown together as 8), 6 reader tests, 9 composer tests, 4 side-effect tests, 6 service-test additions (3 side-effect-dispatch + 3 edit-validation), 4 worker tests. Exact per-test names confirmed via `--reporter=verbose` output listing every `✓` line.
- Approve side effect atomicity (service-level, `InMemoryActionProposalWriter` + a mocked `buildRestockPoApproveSideEffect`): approve stages the side effect exactly once (`sideEffectSpy` called 1×); a retried approve on the same (now-approved) proposal `rejects.toThrow(ActionProposalAlreadyDecidedError)` and the side effect spy count stays at 1 (the writer's staged-status guard fires before the side effect runs again — proves "exactly one PO" in the real Drizzle transaction path); a failing side effect causes the whole `approveProposal` call to reject, and afterward `writer.get()` shows the proposal still `"staged"` with `writer.allDecisions()` containing zero rows for that proposal id — confirmed passing, all 3 green.
- Pure side-effect SQL-shape test (`restock-po-approve-side-effect.test.ts`, fake `tx` with an `insert().values()` spy, no real DB): non-restock kind → `undefined` (no dispatch); a 2-line draft → exactly 1 `purchaseOrders` insert (`status: "draft"`, `createdBy` = the approving user's real id, `supplierName` from draftContent) and exactly 2 `poLines` inserts, every line's `purchaseOrderId` matching the PO's own generated id; `createdBy` asserted to NOT match `/^system:/`; a malformed `draftContent` shape throws — all 4 confirmed passing.
- Edit-body per-kind Zod (`action-proposal-service.test.ts`): an invalid restock `editedContent` (`{ supplierName: "", lines: [] }`) → `editProposal` rejects with `ActionProposalEditValidationError` and the proposal's status is confirmed still `"staged"` (edit was blocked before any transition); a valid restock `editedContent` → `edited`; a non-restock kind with an arbitrary edited shape → still succeeds (no cross-kind schema bleed) — all 3 confirmed passing. Route-level mapping (`server/routes/action-proposals.ts`) added defensively (checks `err.name === "ActionProposalEditValidationError"`, not `instanceof`, specifically so the existing fully-mocked `tests/autopilot/action-proposals.routes.test.ts` — which does not export this class from its `action-proposal-service.js` mock — is not broken by an `instanceof undefined` throw); confirmed via `pnpm exec vitest run tests/autopilot/action-proposals.routes.test.ts` → `8 passed (8)`, unchanged from before this slice.
- Citation grounding — derived-value rejection (the plan's explicit "never cite a derived number" contract): `action-proposal-citation-validator.test.ts`'s new case cites the literal string `"8"` (the summed on-hand total from the reader test's own fixture) as if it were an observed `vt_container_items` row id → `valid: false`, flagged `citation_not_grounded:8` — confirmed passing. `restock-po-composer.test.ts` independently asserts the composed `citedFacts` array never contains `"8"` (the onHand sum) or `"12"` (the computed suggested quantity) as a `sourceId`, and has exactly length 3 (1 inventory-item row + 2 container-item rows) for a 1-item, 2-container-row fixture.
- Worker idempotency + no duplicate audit/metric emission (the §3-established assertion pattern, deliverable D): `autopilot-restock-worker.test.ts` — one flagged item → `{ scanned: 1, staged: 1 }`, exactly 1 row in `writer.findStaged(clinicId, { kind: "restock_po_on_burn" })`; a second scan of the same clinic-day → `staged: 0`, `findStaged` still length 1, AND `incrementMetric` calls filtered to `"autopilot_proposal_staged_total"` has length 1 AND `logAudit` calls filtered to `actionType === "action_proposal_staged"` has length 1 (asserted via `vi.mocked(...).mock.calls.filter(...)`, mirroring the §3-fix-wave pattern) — confirmed passing.
- Reader threshold rule (`restock-burn-reader.test.ts`, InMemory fake): `reorderPoint=10`, summed `onHand=8` across 2 container rows → `flagged: true`, both rows returned in `containerRows` with their real `id`s; `onHand=12` → `flagged: false`; `reorderPoint: null` → item never appears in `result.items` at all; `isActive: false` → same, never appears even with a matching low on-hand; an item with zero container rows → `onHand: 0`, `flagged: true`, `containerRows: []` (no fabricated citation); cross-tenant (clinic B read never sees clinic A's items/rows) — all 6 confirmed passing.
- `pnpm i18n:check` → `✓ locales/en.json and locales/he.json are in deep key parity.` (run after the `autopilotQueue.kinds.restockPoOnBurn.*` addition to both locale files).
- **Job-registry-parity regression found and fixed** (not part of the original task brief, discovered via the full-suite run): adding `restock_po_on_burn`'s worker introduced two REAL new failures unrelated to `tests/autopilot/`: (1) `tests/jobs/job-registry-parity.test.ts`'s inline snapshot (`REGISTRY_STATIC_BULLMQ_JOB_NAMES`) went stale — fixed by adding the new `"scan-restock-burn"` entry in sorted position; (2) `server/lib/job-latency.ts`'s `_ExhaustiveCheck` (a `JobKind`-exhaustiveness compile-time guard) failed once `"scan-restock-burn"` was added to `StaticJobKind` (`server/jobs/registry.ts`) — fixed by adding it to `KNOWN_JOB_KINDS`. Registered a matching `restockBurnScanDefinition` in `server/jobs/definitions/index.ts` (mirrors `sweepEscalationDefinition`'s pattern for a standalone inline-Queue/Worker file). Verified: `pnpm exec vitest run tests/jobs/job-registry-parity.test.ts` → `5 passed | 1 failed` — the ONE remaining failure is `scan-coordinator-reassign` (the §3 sibling worker's job name, never registered), confirmed **pre-existing** via `git stash -u` (removes all uncommitted + untracked changes) → same test run on the untouched base branch → identical single failure, same missing name, same message. Left unfixed (out of this slice's file-ownership scope per "don't refactor outside what was asked"); disclosed as a known gap in `server/jobs/definitions/index.ts`'s new comment block for whoever owns §3 follow-up.
- Full test suite regression check: `pnpm test` before (via `git stash -u`) → `Test Files 32 failed | 632 passed (664)`, `Tests 41 failed | 5755 passed | 11 skipped (5807)`. After (this slice applied) → `Test Files 32 failed | 636 passed (668)`, `Tests 41 failed | 5787 passed | 11 skipped (5839)` — same 32 failed files / 41 failed tests (all pre-existing: DB-integration tests failing on `column "signup_join_code" of relation "vt_clinics" does not exist`, a pre-existing local-DB/migration-drift issue, plus the one disclosed `scan-coordinator-reassign` gap above) — zero net-new failures, +4 test files / +32 tests, all new tests passing.
- Typecheck delta (server): `npx tsc -p tsconfig.server.json --noEmit --pretty false`, sorted, diffed against the same command run on the base branch via `git stash -u` → `diff` exit 0, byte-identical 5-line output both sides (`@aws-sdk/s3-request-presigner`, `socket.io` ×1, `@socket.io/redis-adapter`, 2× implicit-any in `realtime-collab/server.ts`) — zero new server errors. (This diff caught the `job-latency.ts` exhaustiveness regression above before it was fixed — the "after" run initially showed a 6th, new error line; re-run post-fix is the byte-identical result reported here.)
- Typecheck delta (frontend): `npx tsc --noEmit --pretty false`, sorted, diffed the same way → `diff` exit 0, identical 2-line output both sides (`@clerk/localizations`, `socket.io-client`) — zero new frontend errors. No frontend files touched this slice (sanity-run only, per the constraint that no UI files ship in this slice).
- Frozen-surface + read-only-reference check: `git diff feat/2.0-task-1.1-s3-coordinator-reassign -- server/lib/shift-handover-scheduler.ts server/lib/realtime-outbox.ts server/lib/event-publisher.ts server/routes/procurement.ts server/services/restock.service.ts | wc -l` → `0` — none of the frozen surfaces, and neither of the two explicitly read-only reference files (`procurement.ts`, `restock.service.ts`), appear in the diff at all.
- Untracked owner file `docs/deep-research-report (2).md`: confirmed still `??` (untracked) via `git status --porcelain` after every commit in this slice — never staged, never read, never touched.
...
- The RED-before-GREEN process exception for `restock-po-approve-side-effect.ts` (implementation preceded its dedicated test file) — see the Evidence section above.
- The job-registry-parity/`job-latency.ts` fixes above were not explicitly requested deliverables but were required to avoid shipping a newly-red `pnpm test` run; the pre-existing `scan-coordinator-reassign` half of that same gap was deliberately left alone (out of file-ownership scope for this slice).
...
## 2026-07-22 — Task 1.1 §2 `shift_handover_draft` — branch feat/2.0-task-1.1-s2-handover-draft

**Claim:** Implemented the `shift_handover_draft` autopilot proposal type per plan §2, SHADOW STAGING ONLY: `aggregateDeltas` export (one-keyword diff), pure composer over R-SH-F1's own `resolveShiftWindow`/`aggregateDeltas`, a 10-min scan worker registered additively in `start-schedulers.ts`, job-registry entries, per-kind edit Zod, citation-validator case, i18n keys in both locales — all RED-before-GREEN. R-SH-F1's auto-publish path (`server/lib/shift-handover-scheduler.ts`, its registration) left completely untouched; both artifacts now run in parallel for the same ended session. §0(c) per-org cutover explicitly NOT built (follow-up after §6).

**Evidence:**
- Branch: created off `feat/2.0-task-1.1-s5-crash-cart-drift` via `git checkout -b feat/2.0-task-1.1-s2-handover-draft` — confirmed via `git log --oneline feat/2.0-task-1.1-s5-crash-cart-drift..HEAD` → 3 commits (`ee3ed96aa`, `e0cd6f8a7`, `6bb9490bf`).
- RED confirmed BEFORE any production file was written: `pnpm exec vitest run tests/autopilot/handover-draft-composer.test.ts tests/autopilot/autopilot-handover-worker.test.ts tests/autopilot/action-proposal-citation-validator.test.ts tests/autopilot/action-proposal-service.test.ts tests/autopilot/action-proposal-types.test.ts tests/jobs/job-registry-parity.test.ts` → `11 failed | 47 passed (58)` — module-not-found for the composer and worker files, `TypeError: Cannot read properties of undefined (reading 'safeParse')` for `shiftHandoverDraftEditedContentSchema`, `expected true to be false` for `validateEditedContentForKind("shift_handover_draft", …)`, and two inline-snapshot mismatches missing `scan-handover-draft`.
- Also caught by RED (not anticipated at test-authoring time, fixed as part of the same TDD cycle): once `shiftHandoverDraftEditedContentSchema` landed, two PRE-EXISTING generic tests in `action-proposal-service.test.ts` that used the default `shift_handover_draft` kind with an arbitrary `editedContent` shape (`{ note: "adjusted" }`, `{ anything: "goes" }`) started failing for real (`ActionProposalEditValidationError`) — both repointed to `coordinator_reassign_off_roster` (the one remaining kind with no registered schema), a behavior-neutral test-only fix.
- GREEN, full suite: `pnpm exec vitest run tests/autopilot/ tests/jobs/` → `Test Files 26 passed (26)` / `Tests 177 passed (177)`.
- Also ran the pre-existing shift-handover suites this slice exports a private function from (per the task's explicit instruction): `pnpm exec vitest run tests/shift-handover-contract.test.ts tests/shift-handover-generator.test.ts tests/shift-handover-observed.test.ts tests/shift-handover-patient-worklist.test.ts tests/shift-session-auto-checkout.test.ts tests/routes-registration-contract-slice7.test.ts` → `4 failed | 2 passed (6 files)`, `18 failed | 11 passed (29 tests)`. Confirmed PRE-EXISTING and unrelated via `git stash -u` + rerun on the unmodified base branch: identical failures — `tests/shift-handover-generator.test.ts`/`tests/shift-handover-observed.test.ts`/`tests/shift-handover-patient-worklist.test.ts` all fail on `error: column "signup_join_code" of relation "vt_clinics" does not exist` (local test-DB migrations out of sync with schema, a live-DB integration issue unrelated to this slice's code), and `tests/routes-registration-contract-slice7.test.ts` fails on `Cannot find package '@aws-sdk/s3-request-presigner'` (an unrelated missing dependency in this sandbox, imported transitively via `server/routes/users.ts` → `server/lib/object-storage.ts`, nothing this slice touches). The two suites that don't need a live DB (`shift-handover-contract.test.ts`, `shift-session-auto-checkout.test.ts`) pass: `2 passed (2 files)` / `11 passed`.
- Per-origin citation mapping confirmed by reading `server/schema/ops.ts:270-290` and `server/lib/audit.ts:364-369`: `vt_audit_logs.id` is a TEXT primary key populated via `randomUUID()` (always hyphenated `8-4-4-4-12`), `vt_event_outbox.id` is a `bigserial` primary key that `aggregateDeltas` stringifies (`String(r.id)`, always pure digits) — so `/^\d+$/.test(sourceId)` deterministically discriminates `vt_event_outbox` from `vt_audit_logs`, never a heuristic guess (a UUID can never match that pattern). Implemented as `isOutboxSourcedId` in `handover-draft-composer.ts`, asserted by two dedicated composer tests (one audit-sourced UUID entry, one outbox-sourced numeric entry).
- Empty-deltas decision: read `generateShiftHandover` (`server/lib/shift-handover-generator.ts:464-530`) — it has NO "skip if deltas are empty" branch; it always inserts a `vt_shift_handover` row (revision 1) regardless of delta count. Mirrored exactly: `composeHandoverDraftProposal` never throws on an empty `ShiftHandoverDeltas` (unlike the signal-detection kinds §3–§5), and `runHandoverDraftScan` stages a proposal for every candidate session whether or not it has deltas — asserted by `tests/autopilot/handover-draft-composer.test.ts`'s "R-SH-F1 parity" case and `tests/autopilot/autopilot-handover-worker.test.ts`'s "session with ZERO deltas still gets a staged (empty) handover draft" case.
- `handover-artifact-panel.tsx` grepped per the plan's §2 note: `grep -n "export" src/components/handover-artifact-panel.tsx` → only `HandoverArtifactPanel` (the whole component), `HandoverArtifactViewModel`, `HandoverPanelState` are exported — NO reusable delta-rendering sub-components exist today. A future §6 `HandoverDraftCard.tsx` cannot reuse pieces of this file as-is; it would need to either duplicate rendering logic or a follow-up refactor of `handover-artifact-panel.tsx` to extract sub-components first. Flagged for whoever executes §6, not resolved here (out of scope — no UI was built in this slice).
- Typecheck delta: `npx tsc --noEmit --pretty false` (frontend) and `npx tsc -p tsconfig.server.json --noEmit --pretty false` (server) captured on the clean base branch (`git stash -u` round-trip, confirmed both new untracked files were stashed away for the "before" run) and on this branch's final tree; `diff <(sort before) <(sort after)` → empty output both times (zero new errors; the 2 frontend + 5 server pre-existing errors — missing `@aws-sdk/s3-request-presigner`, `socket.io`, `@socket.io/redis-adapter` modules — are present identically on the base branch, an unrelated sandbox dependency-install gap).
- i18n: `locales/en.json` / `locales/he.json` both gained `autopilotQueue.kinds.shiftHandoverDraft.{title,summaryTemplate}`. Command: `pnpm i18n:check` → `✓ locales/en.json and locales/he.json are in deep key parity.`
- Frozen-surface diff proof: `git diff --stat feat/2.0-task-1.1-s5-crash-cart-drift..HEAD -- server/lib/shift-handover-generator.ts server/lib/shift-handover-scheduler.ts` → `server/lib/shift-handover-generator.ts | 2 +-` only (`shift-handover-scheduler.ts` absent from the stat entirely — zero-line diff); the one-line diff itself: `-async function aggregateDeltas(` / `+export async function aggregateDeltas(`, nothing else changed in that file.
- `docs/deep-research-report (2).md` never staged: `git status --short` shows it as `??` (untracked) at every commit point; explicit `git add <paths>` used for every commit (never `git add -A`/`.`).
- `pnpm tenant:lint:touched` (warn-only) → 5 pre-existing warnings, all in files from prior slices (`action-proposal-writer.port.ts`, `crash-cart-drift-reader.port.ts`) — zero warnings on any file this slice added or modified.
- No push, no PR, no amend, no `--no-verify` used at any point.

**Skipped / deferred (explicitly out of scope per the task brief, not overlooked):**
- No §0(c) per-org cutover mechanism (policy key, scheduler skip, auto-publish-on-timeout, `auto_published_on_timeout` audit kind) — explicitly deferred to after the §6 UI ships, per the task's binding scope boundary.
- No `src/features/autopilot/cards/HandoverDraftCard.tsx` or any other UI file — deferred to the §6 UI-shell slice (matches §4/§5's precedent); `handover-artifact-panel.tsx` has no reusable sub-components today (see above), a real gap for whoever builds that card.
- `observedSignals` / `patientWorklist` not included in `draftContent` — both require DB I/O this composer's "pure, no I/O" contract and the one-line-diff constraint on `shift-handover-generator.ts` put out of scope (documented in the composer's file header as a disclosed gap, not silently omitted).
- No new `AuditActionType` or `MetricName` union members — the shared §1 `stageProposal`/`decide()` calls already cover all 4 kinds generically.
