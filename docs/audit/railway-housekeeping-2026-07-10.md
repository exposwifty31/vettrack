...
| `dboy3156/VetTrack` GitHub repo | No Railway service deploys from it anymore (production Worker disconnected; Staging repointed to `exposwifty31/vettrack`). It had silently frozen Staging since May and the Worker since June 12. | Archive the repo. Also consider removing the stale `gitlab` remote from local clones. |
...
| Item | Finding | Suggested action |
|------|---------|------------------|
| `Staging ` environment name | Has a **trailing space**. Any CLI `--environment` usage must quote it exactly. | Rename to `staging` after confirming no token or script references the exact padded string. |
