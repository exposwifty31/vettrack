...
| **F-2** damaged-return third button on-device | **VERIFIED CORRECT** — source unconditional + `return-damaged.test.tsx` 4/4 + renders in jsdom; the on-device miss was a WKWebView bundle-cache artifact (re-confirm on a clean `cap:build:native`). No code change. |
...
| **Role-onboarding** (owner request) | **SHIPPED** — two-option chips (vet/tech), vet gated on a doctor/license number, auto-promote-on-approval with admin override + `422 VET_LICENSE_REQUIRED` gate, migration 163. Unit-covered (approval-role, role-chips, pending-users, provisioning). |
