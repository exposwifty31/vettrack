...
Both deletions are **correct verdicts** and neither was a one-file change. §C.3 below is
the exact companion-edit set: **seven test files**, plus `src/lib/query-keys/registry.ts`,
which is a **generated** drift registry rather than a test and is regenerated by
`pnpm query-keys:audit -- --write-registry` instead of hand-edited.

At the time this register was written, all seven pinned the routes by asserting on
**source text** — one of them via a module-scope `readFileSync` that errors at collection,
so removing a single file turned the suite red. That is why the patches were reported
rather than applied: the lane that wrote this was scoped out of test files.

Both patch sets have since been applied in full. Two of the seven no longer assert on
source text at all: `equipment-readiness-wedge-smoke.test.ts` and
`routes-registration-contract-slice7.test.ts` now drive `registerApiRoutes` with a
recording stub and assert on the **mounts that actually happened**, so a re-added mount
fails there regardless of what any file's text says. `wf2-proof/D6.md` retains the
original patch text for reference.
