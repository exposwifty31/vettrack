...
- **massive-01 passive RFID-gate tracking** — **core functionality (owner: not optional); technology LOCKED to RFID-gate** (chokepoint egress / last-seen), **not BLE/RTLS** — owner decision 2026-07-12 per `R-M1-PRE` (lowest TCO at clinic scale; `docs/business-case/2026-07-12-massive-01-passive-tracking-cost-benefit.md`). **Hardware rollout** stays gated on the manager go (owner brings the `R-M1-PRE` §6 clinic numbers). **The software plan is now unblocked and MUST ship as a genuine end-to-end feature — explicitly NOT the current inert scaffolding.** Today RFID is scoped-but-not-wired (`server/services/rfid-readers.service.ts`, the `/admin/rfid-readers` route, and `docs/{equipment-readiness-rfid-gap-analysis,equipment-readiness-wedge-master-execution-plan,rfid-smoke}.md`). Its **dedicated SDD spec-plan (`R-M1` — next deliverable)** must wire **and e2e-verify** the full stack:
  - **Backend:** reader-payload **ingest route** (clinic-scoped auth, rate-limited), `vt_location_signals` table (`clinicId, equipmentId, readerId, observedAt, source ∈ ble|rfid|scan`), evidence-resolver **weighting** (passive signal = a new weighted input; scan stays first-class).
  - **Frontend:** `src/lib/api.ts` fn + `src/types/` type; **admin reader-management UI that actually performs CRUD** (replaces the inert route).
  - **UX/UI:** RFID **last-seen / egress** surfaced in locate (small-01), readiness/last-seen chips (small-02), `/board`, and equipment-detail — real data, not a placeholder.
  - **e2e verification (the acceptance bar):** simulated reader payloads → resolver returns the correct room + custodian, surfaced in the UI; a scan-only clinic snapshot is **byte-for-byte unchanged** (golden test); cross-clinic reader IDs rejected; partial coverage degrades gracefully to last-known (no "unknown" regressions). Additive only; `clinicId` on every signal read/write.
...
**Still open / gated:**
- **massive-01** — **RFID-gate LOCKED (owner 2026-07-12)**; `R-M1-PRE` cost/benefit delivered (`docs/business-case/2026-07-12-massive-01-passive-tracking-cost-benefit.md`). Hardware rollout still gated on the manager go (owner supplies the §6 clinic inputs). **Software is a green-lit build:** the next deliverable is the dedicated **RFID-gate e2e SDD spec-plan (`R-M1`)** — full backend + frontend + UX/UI wiring + e2e verification, replacing today's inert scaffolding. Grounding investigation of the current RFID state in progress.
