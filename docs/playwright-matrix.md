...
| `PW_SUITE=workday` | same | `e2e/simulation/workday.spec.ts` | Local app at `http://127.0.0.1:3001` | Run by `e2e-simulation-nightly.yml`, which builds/migrates/seeds locally; the deleted workflow was the hosted **staging** one |
...
| `tests/e2e/simulation/workday.spec.ts` | `PW_SUITE=workday` | Compressed day against a LOCAL app — `e2e-simulation-nightly.yml` still runs it nightly; `workday-simulation-nightly.yml` (hosted, staging-targeted) was deleted |
