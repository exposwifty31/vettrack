...
4. When debugging NFC pipelines, inspect recent scan activity directly: `psql -d "$DATABASE_URL" -c "SELECT id, clinic_id, equipment_id, status, timestamp FROM vt_scan_logs ORDER BY timestamp DESC LIMIT 30;"`.
...
| Script | Purpose |
|--------|---------|
| `psql -d "$DATABASE_URL"` | Ad-hoc audit of `vt_scan_logs` and orphaned billable dispenses |
