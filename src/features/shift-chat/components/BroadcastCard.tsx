...
export function BroadcastCard({ message, currentUserId, isSender, onAck }: BroadcastCardProps) {
  ...
  return (
    <div className="rounded-xl border border-[var(--brand-border)] bg-[var(--brand-soft)] p-3 my-1">
      ...
      {template?.subtitle && (
        <div className="text-xs text-[var(--brand)] mb-3">{template.subtitle}</div>
      )}

      {isSender && totalTechs > 0 && (
        <div className="mt-1">
          <div className="h-1 bg-[var(--brand-soft)] rounded-full overflow-hidden mb-2">
            <div
      ...
      {!isSender && !myAck && (
        <div className="flex gap-2 mt-1">
          <button
            ...
            type="button"
            className="flex-1 bg-indigo-600 hover:bg-[var(--brand-deep)] text-[var(--on-ink)] rounded-lg py-2 text-sm font-bold transition-colors"
          >
            ✓ קיבלתי — בדרך
          </button>
          <button
            ...
            type="button"
            className="bg-transparent border border-[var(--brand-border)] text-[var(--brand)] rounded-lg px-3 py-2 text-xs transition-colors hover:border-[var(--brand)]"
          >
            ⏱ 5 דק׳
          </button>
        </div>
