...
origin: fork
upstream: https://github.com/logesh-kumar/publish-mobile-app
---

> **VetTrack fork:** see [FORK.md](FORK.md). Reviewer login uses Clerk — follow [RESUBMISSION_RUNBOOK.md](../../../RESUBMISSION_RUNBOOK.md), not `create-app-review-user.ts` (Supabase-only).
...
10. **Reviewer user**
    - **VetTrack (Clerk):** follow [RESUBMISSION_RUNBOOK.md](../../../RESUBMISSION_RUNBOOK.md) §C — verify `reviewer@vettrack.uk` login, Client Trust off, admin role + English locale. Run `./scripts/verify-resubmission.sh` before submit.
    - **Other stacks (Supabase):** run [scripts/create-app-review-user.ts](scripts/create-app-review-user.ts) — provisions reviewer in Supabase if detected; stores creds in `ios/.env.review`. Falls back to manual instructions if not Supabase.
