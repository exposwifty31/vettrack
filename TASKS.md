...
> stores — *iOS half DONE 2026-09-03: App Store 1.3.0 is RN build 30, review submission COMPLETE; verified 2026-09-09 via `asc versions view --include-build`* — Android alpha draft 10302 still has zero testers), tracked in `~/.claude/plans/store-submission-runbook.md`. It is not card-shaped and does
...
- TASK (ops, security) — **OPEN SECURITY EXCEPTION, owner `exposwifty31`, decided 2026-09-11, expires 2026-10-11**: `DB_SSL_REJECT_UNAUTHORIZED` is `"false"` in production on VetTrack (and therefore on Worker, which references it), so `getPgSslConfig()` in `server/lib/postgresql.ts` does NOT verify the managed-Postgres certificate. Found 2026-09-11 when an exact-`"true"` check in the worker gate crash-looped the Worker. Flip the Railway variable to `"true"` (verify Railway's Postgres cert chain is trusted by the Node image first, or supply `ssl.ca`), redeploy Worker then VetTrack, confirm `/api/health` `db: ok`; THEN tighten both `validateEnv()` and `validateWorkerEnv()` in `server/lib/envValidation.ts` to require the exact string, in one change.
- ~~TASK (owner): decide the `UPLOAD_KEY_CERT_FINGERPRINT` mismatch~~ **DONE 2026-09-11:** `keytool -printcert -jarfile` on the EAS AAB 10302 reads `38:31:…` (= Play Console's Upload key = the EAS-managed keystore); `93:34:…` is the Capacitor shell's local upload key. `server/lib/well-known-assetlinks.ts` now serves the list `UPLOAD_KEY_CERT_FINGERPRINTS` (both) + the Play App Signing key from the env var. Original text: decide the mismatch in `server/lib/well-known-assetlinks.ts` — the constant (`93:34:…`) is not the upload key Play Console lists (`38:31:…`); store installs verify through the Play App Signing key either way. Update, remove, or document why. See `docs/runbooks/o2-eas-keystore.md`.
