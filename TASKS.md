...
- TASK (ops, security): `DB_SSL_REJECT_UNAUTHORIZED` is `"false"` in production on VetTrack (and therefore on Worker, which references it), so `getPgSslConfig()` in `server/lib/postgresql.ts` does NOT verify the managed-Postgres certificate. Found 2026-09-11 when an exact-`"true"` check in the worker gate crash-looped the Worker. Flip the Railway variable to `"true"` (verify Railway's Postgres cert chain is trusted by the Node image first, or supply `ssl.ca`), redeploy Worker then VetTrack, confirm `/api/health` `db: ok`; THEN tighten both `validateEnv()` and `validateWorkerEnv()` in `server/lib/envValidation.ts` to require the exact string, in one change.
