 ...
 *   1. reload only on a CONFIRMED byte-different service worker (not a waiting
 *      worker, not peer split-version gossip),
 *   2. DEFER while a Code Blue is active and fire only when the server snapshot
 *      goes calm (codeBlueSession dropped),
...
import { isConfirmedNewWorker, useBoardAutoReload } from "@/board/useBoardAutoReload";
...
describe("useBoardAutoReload — reload + defer machine", () => {
  ...
  it("ignores waiting-worker and peer-gossip events (no reload)", () => {
    const qc = qcWith(null);
    renderHook(() => useBoardAutoReload(), { wrapper: wrapperFor(qc) });
    act(() => dispatchSwUpdate({ worker: null, buildTag: OTHER_TAG })); // peer gossip
    act(() => dispatchSwUpdate({ worker: worker(), buildTag: null })); // waiting worker
    expect(recoverSpy).not.toHaveBeenCalled();
    expect(telemetry).not.toHaveBeenCalled();
  });
});
