...
// Pin the clock to NOW. Every predicate under test reads the wall clock
// (`isInactive` -> subDays(new Date(), ...), and the two server ones do the
// same), while the fixtures are built from a frozen NOW. Without this the
// boundary assertion below drifts one day per day and starts failing the day
// after the slice is written — which is exactly what it did.
beforeAll(() => {
